...
class Lsoft_setting
{
 //sms config update form for sending sms
    ...
    public function send_sms($order_no=null,$customer_id=null,$type=null){
        ...
        $CI =& get_instance();
        $CI->load->model('setting/Sms_model','sms_model');
		...
		//print_r($sms_template);
		 $sms_type= strtolower($sms_template->type);
         if($sms_type == "neworder" || $sms_type == "completeorder" || $sms_type == "processing" || $sms_type == "cancel" || $sms_type == "otp" || $sms_type == "orderaccpet"){             
        ...
        ****************************/
		...
		if(4 == $gateway->id ){
			...
			if(!empty($message) && !empty($phone)){
    			...
    			$urltopost = "http://sms.bdtask.com/smsapi";
			    $datatopost = array (
					"api_key"	=> $gateway->user_name,
					"type" 		=> 'unicode',
					"senderid" 	=> $gateway->userid,
					"msg" 		=> $message,
					"contacts" 	=> $phone
			);
