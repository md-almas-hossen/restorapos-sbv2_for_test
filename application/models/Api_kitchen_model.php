...
class Api_kitchen_model extends CI_Model
{
   ...
   public function read2($select_items, $table, $orderby, $where_array)
	...
	public function allkitchenorderlist(){
		$Today=date('Y-m-d');
		$condition="(customer_order.order_status=1 OR customer_order.order_status=2)  AND customer_order.order_date='".$Today."'";
	    $this->db->select('customer_order.*,item_foods.kitchenid,item_foods.is_customqty,order_menu.menu_id,customer_info.customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
		...
		public function kitchen_tokenlist($id,$status){
			$cdate=date('Y-m-d');
			if($status>1){
			$where="customer_order.order_date = '".$cdate."'";
			}else{
			    $where="customer_order.order_status='".$status."' AND customer_order.order_date = '".$cdate."'";
			}
			...
			public function apptokenorderkitchentokenpending($id,$kitchen){
				$this->db->select('tbl_apptokenupdate.*,SUM(tbl_apptokenupdate.add_qty) as tqty,item_foods.ProductsID,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,item_foods.component,item_foods.descrip,item_foods.itemnotes,variant.variantid,variant.variantName,variant.price');
				...
				$this->db->where('tbl_apptokenupdate.foodstatus=',0);
				...
				public function apptokenorderkitchentokenprocess($id,$kitchen){
					$this->db->select('tbl_apptokenupdate.*,SUM(tbl_apptokenupdate.add_qty) as tqty,item_foods.ProductsID,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,item_foods.component,item_foods.descrip,item_foods.itemnotes,variant.variantid,variant.variantName,variant.price');
					$this->db->from('tbl_apptokenupdate');
					$this->db->join('item_foods','tbl_apptokenupdate.menuid=item_foods.ProductsID','left');
					$this->db->join('variant','tbl_apptokenupdate.varientid=variant.variantid','left');
					$this->db->where('tbl_apptokenupdate.ordid',$id);
					$this->db->where('tbl_apptokenupdate.foodstatus=',1);
					$this->db->where('tbl_apptokenupdate.del_qty',0);
					$this->db->where('item_foods.kitchenid',$kitchen);
					$this->db->group_by('tbl_apptokenupdate.addonsuid');
					$query = $this->db->get();
			...
			public function apptokenorderkitchentoken($id,$kitchen){
				$this->db->select('tbl_apptokenupdate.*,SUM(tbl_apptokenupdate.add_qty) as tqty,item_foods.ProductsID,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,item_foods.component,item_foods.descrip,item_foods.itemnotes,variant.variantid,variant.variantName,variant.price');
				$this->db->from('tbl_apptokenupdate');
				$this->db->join('item_foods','tbl_apptokenupdate.menuid=item_foods.ProductsID','left');
				$this->db->join('variant','tbl_apptokenupdate.varientid=variant.variantid','left');
				...
				public function apptokenorderkitchentokenready($id,$kitchen){
					$this->db->select('tbl_apptokenupdate.*,SUM(tbl_apptokenupdate.add_qty) as tqty,item_foods.ProductsID,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,item_foods.component,item_foods.descrip,item_foods.itemnotes,variant.variantid,variant.variantName,variant.price');
					$this->db->from('tbl_apptokenupdate');
					$this->db->join('item_foods','tbl_apptokenupdate.menuid=item_foods.ProductsID','left');
					$this->db->join('variant','tbl_apptokenupdate.varientid=variant.variantid','left');
	...
	public function orderlist($kitchenid){
		$Today=date('Y-m-d');
		$condition="(customer_order.order_status=1 OR customer_order.order_status=2  OR customer_order.order_status=3)  AND customer_order.order_date='".$Today."' AND item_foods.kitchenid in($kitchenid)";
	    $this->db->select('customer_order.*,item_foods.kitchenid,item_foods.is_customqty,order_menu.menu_id,customer_info.customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
            $this->db->from('customer_order');
    		$this->db->join('customer_info','customer_order.customer_id=customer_info.customer_id','left');
    		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
	...
	public function readyorderlist($kitchenid){
		$Today=date('Y-m-d');
		$condition="(customer_order.order_status=2 OR customer_order.order_status=3)  AND customer_order.order_date='".$Today."' AND item_foods.kitchenid in($kitchenid)";
	    $this->db->select('customer_order.*,item_foods.kitchenid,item_foods.is_customqty,order_menu.menu_id,customer_info.customer_name,customer_type.customer_type,employee_history.first_name,employee_history.last_name,rest_table.tablename');
            $this->db->from('customer_order');
    		$this->db->join('customer_info','customer_order.customer_id=customer_info.customer_id','left');
    		$this->db->join('customer_type','customer_order.cutomertype=customer_type.customer_type_id','left');
	...
	public function foodlist($CategoryID){
		$this->db->select('item_foods.ProductsID,item_foods.ProductName,item_foods.is_customqty,item_foods.ProductImage,item_foods.component,item_foods.itemnotes,item_foods.descrip,item_foods.productvat,item_foods.OffersRate,item_foods.offerIsavailable,item_foods.offerstartdate,item_foods.offerendate,item_foods.ProductsIsActive,variant.variantid,variant.variantName,variant.price');
	...
	public function allkitchenincomminglist(){
			
			
    		$cdate=date('Y-m-d');
    		$where="customer_order.waiter_id>0 AND customer_order.order_date='".$cdate."' AND (customer_order.order_status=1 OR customer_order.order_status=2)";
	...
	public function allincommingkitchen($orderid,$kitchenid){
			...
			$allitemid=$this->db->select('menu_id,varientid')->from('order_menu')->where('order_id',$orderid)->get()->result();
			$allvalues="";
			foreach($allitemid as $ids){
			    $actual_kitchen=$this->db->select('kitchenid,is_customqty')->from('item_foods')->where('ProductsID',$ids->menu_id)->get()->row();
				...
				$this->db->where('varient',$ids->varientid);
    			$query = $this->db->get();
	...
	public function viewincommingkitchen($orderid,$kitchenid){
    		...
    		$this->db->select('order_menu.*,item_foods.ProductName,item_foods.kitchenid,item_foods.is_customqty,variant.variantid,variant.variantName,variant.price,customer_order.order_status');
