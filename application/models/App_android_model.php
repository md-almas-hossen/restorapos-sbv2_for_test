...
class App_android_model extends CI_Model
{
   ...
   public function read2($select_items, $table, $orderby, $where_array)
    ...
    public function authenticate_user($table, $data)
    {
        ...
        $this->db->select("user.id,user.firstname, user.lastname, user.email,user.is_admin");
	...
	public function allfoodlist(){
		...
		// $this->db->select('item_foods.ProductsID,item_foods.CategoryID,item_foods.ProductName,item_foods.foodcode,item_foods.ProductImage,item_foods.component,item_foods.itemnotes,item_foods.descrip,item_foods.productvat,item_foods.OffersRate,item_foods.offerIsavailable,item_foods.offerstartdate,item_foods.offerendate,item_foods.ProductsIsActive,variant.variantid,variant.variantName,variant.price');
	...
	public function foodlistdesktop(){
		$this->db->select('*');
        $this->db->from('item_foods');
	...
	public function addonslist($istopping)
	{ 
		$this->db->select('*');
        $this->db->from('add_ons');
		$this->db->where('istopping',$istopping);
		$query = $this->db->get();
		$addons=$query->result();
	    return $addons;
	}
	...
	public function toppingmenulist()
	{ 
		$this->db->select('*');
   ...
   public function allincomminglist(){
    	...
    	public function get_ongoingorder(){
    	    	...
    	    	$userinfo=$this->db->query("SELECT * FROM user WHERE id=$uid");
    	    	//echo $this->db->last_query();
	           $userlog= $userinfo->row();
	//print_r($userlog);
		/*$cdate=date('Y-m-d');
		...
		$where="customer_order.order_date Between '".$cdate."' AND '".$today."' AND (customer_order.ordered_by='".$uid."' OR customer_order.ordered_by=0) AND ((customer_order.order_status = 1 OR customer_order.order_status = 2 OR customer_order.order_status = 3) AND ((customer_order.cutomertype = 99 AND customer_order.orderacceptreject = 1) || (customer_order.cutomertype = 3 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 4 || customer_order.orderacceptreject != 1) || (customer_order.cutomertype = 1 || customer_order.orderacceptreject != 1)))";
		}
	...
	{
		 $data = $this->db->select("*")->from('customer_info')->where('is_active',1)->get()->result();
		 return $data;
	}
	public function tablelist()
	{
		 $data = $this->db->select("*")->from('rest_table')->get()->result();
		 return $data;
	}
	...
	public function read_allapioneday($select_items, $table, $orderby,$delitem="",$stype="",$val="")
		{	$today=date('Y-m-d');
			$condi="order_date Between '".$delitem."' AND '".$today."'";
			$this->db->select($select_items);
			$this->db->from($table);
			if($delitem!=""){
			$this->db->where($condi);
	...
	public function insert_product($foodid,$vid,$foodqty,$receipe,$saveid)
	{
		$p_id = $foodid;
		$newdate= date('Y-m-d');
		$exdate= date('Y-m-d');
	...
	public function margeview($id){
		$this->db->select('customer_order.*,order_menu.*,item_foods.ProductName,item_foods.is_customqty,item_foods.price_editable,variant.variantid,variant.variantName,variant.price');
        $this->db->from('customer_order');		
 ...
 public function collectcash($id,$tdate){
		...
		$this->db->select('bill.*,multipay_bill.payment_method_id,SUM(multipay_bill.amount) as totalamount,payment_method.payment_method');
        $this->db->from('multipay_bill');
		$this->db->join('bill','bill.order_id=multipay_bill.order_id','left');
	...
	public function collectcashsummery($id,$tdate,$crdate){
		$where="bill.create_at Between '$tdate' AND '$crdate'";
		$this->db->select('bill.*,multipay_bill.payment_method_id,SUM(multipay_bill.amount) as totalamount,payment_method.payment_method, SUM(multipay_bill.change_amount) as total_change_amount');
        $this->db->from('multipay_bill');
		$this->db->join('bill','bill.order_id=multipay_bill.order_id','left');
		$this->db->join('payment_method','payment_method.payment_method_id=multipay_bill.payment_method_id','left');
		$this->db->where('bill.create_by',$id);
	...
	public function collectcashreturnsummery($id,$tdate,$crdate){
		$where="bill.create_at Between '$tdate' AND '$crdate'";
		$this->db->select('bill.*,multipay_bill.payment_type_id,SUM(multipay_bill.amount) as totalreturn,payment_method.payment_method,sale_return.order_id');
        $this->db->from('multipay_bill');
		$this->db->join('bill','bill.order_id=multipay_bill.order_id','left');
		$this->db->join('payment_method','payment_method.payment_method_id=multipay_bill.payment_type_id','left');
	...
	public function printerorder(){
		...
		// public function collectcash_for_android_pos($id,$tdate){
	...
	public function changecash_for_android_pos($id,$tdate){
	...
	public function allm_mobile_payment_methods(){
		$this->db->select('*');
        $this->db->from('tbl_mobilepmethod');
		$query = $this->db->get();
		return $orderdetails=$query->result();
	}
