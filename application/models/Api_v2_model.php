...
class Api_v2_model extends CI_Model
{
   ...
   public function authenticate_user($table, $data)
    {
        $where="is_active =1 AND (customer_email = '".$data['customer_email']."' OR customer_phone = '".$data['customer_email']."')";
		$Type = $data['customer_email'];
        $Password = $data['password'];
        $this->db->select("customer_id,cuntomer_no,membership_type,customer_name,customer_email,password,customer_token,customer_address,customer_phone,customer_picture,favorite_delivery_address,crdate,is_active");
		$this->db->where($where);
	...
	public function checkEmailOrPhoneIsRegistered($table, $data)
    {
		...
		$this->db->or_where('customer_phone', $data['customer_email']);
        $query = $this->db->get($table)->row();
        $num_rows = $this->db->count_all_results();
	...
	public function allfooditem($catid=null,$start=null,$limit=null){
	...
	public function orderitem($orderid,$customerid){
		...
		foreach($cartArray as $item){
			...
			if($item->addons==1){
				...
				$checkgroup=$this->db->select('isgroup,OffersRate')->from('item_foods')->where('ProductsID',$item->ProductsID)->get()->row();
				if($checkgroup->isgroup==1){
						...
						foreach($groupinfo as $grouprow){
								...
								'notes'					=>  $item->itemnote,
								'price'					=>	$item->price,
								'itemdiscount'			=>	$checkgroup->OffersRate,
