...
class App_desktop_model extends CI_Model
{
	...
	public function printerorder()
		{
			...
			$today=date('Y-m-d');
			$condi="order_date Between '".$date."' AND '".$today."'";
			$sql="SELECT * FROM customer_order Where order_date Between '".$delitem."' AND '".$today."' AND invoiceprint!=0 AND splitpay_status=0 AND customer_order.marge_order_id IS NULL UNION SELECT * FROM customer_order Where invoiceprint!=0 AND splitpay_status=0 AND customer_order.marge_order_id IS NOT NULL GROUP BY customer_order.marge_order_id order by order_id ASC";
	...
	public function customerorder($id){
		...
		$this->db->select('order_menu.*,item_foods.ProductsID,item_foods.ProductName,item_foods.kitchenid,item_foods.cookedtime,item_foods.is_customqty,variant.variantid,variant.variantName,variant.price as vprice');
        $this->db->from('order_menu');
		$this->db->join('item_foods','order_menu.menu_id=item_foods.ProductsID','left');
		$this->db->join('variant','order_menu.varientid=variant.variantid','left');
		$this->db->where($cond);
