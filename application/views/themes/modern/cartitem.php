...
$totalqty=0;
			$totalcartamount=0;
if($this->cart->contents()>0){ $totalqty= count($this->cart->contents()); $totalcartamount=$this->cart->total();} ;?>
					...
					$multiplletax = array();
						 ...
						 $i=0; 
                                    ...
                                    $tx++;  
										...
										$addon_price=($getaddonsdata['price']*$addonsqtyarray[$addn]);
										
										$avatcalc= Vatclaculation($addon_price,$getaddonsdata[$fildaname]);
                                        ...
                                        $multiplletax[$fildaname] = $multiplletax[$fildaname]+$avatcalc;
										  ...
										  $avatcalc= Vatclaculation($addon_price,$taxainfo['default_value']);
										...
										<div class="d-flex pb-1 border-dash justify-content-between align-items-center">
											<div class="text-start me-2">
												...
												<div class="fs-13 lh-sm">
													...
													<?php if(!empty($item['addonsid'])){?><div><span class="fw-600"><?php echo display('addons_name')?>:</span> <span class="text-muted"><?php echo numbershow($item['addonname'], $storeinfo->showdecimal);?>(<?php if($this->storecurrency->position==1){echo $this->storecurrency->curr_icon;}?><?php echo numbershow($item['addontpr'], $storeinfo->showdecimal);?><?php if($this->storecurrency->position==2){echo $this->storecurrency->curr_icon;}?>)</span></div><?php } ?>
                                                     <?php if(!empty($item['toppingid'])){?><div><span class="fw-600"><?php echo display('addons_name')?>:</span> <span class="text-muted"><?php echo numbershow($item['toppingname'], $storeinfo->showdecimal);?>(<?php if($this->storecurrency->position==1){echo $this->storecurrency->curr_icon;}?><?php echo numbershow($item['toppingprice'], $storeinfo->showdecimal);?><?php if($this->storecurrency->position==2){echo $this->storecurrency->curr_icon;}?>)</span></div><?php } ?>
												</div>
			...
			<div class="p-0 mt-auto border-top">
                ...
                <div class="p-0 mt-auto border-top">
							...
							<div class="align-items-center border-top d-flex fw-600 justify-content-between px-3 py-2">
								<div><?php echo display('total') ?></div>
								<div><?php if($this->storecurrency->position==1){echo $this->storecurrency->curr_icon;}?>
