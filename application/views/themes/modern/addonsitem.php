...
?>
<div class="modal-header bg-green text-white" style="<?php echo $button_and_bg_color_css;?>">
  <h5 class="modal-title" id="addtocartModalLabel"><?php echo display('food_details'); ?></h5>
...
<div class="modal-body p-0">
  <div class="d-flex justify-content-between p-4">
    ...
    <input name="itemprice" type="hidden" value="<?php echo numbershow($item->price, $storeinfo->showdecimal, '.', ''); ?>" id="itemprice_1<?php echo $type;?>" />
    <input type="hidden" name="cartpage" id="cartpage_1<?php echo $type;?>" value="<?php if($type=="menu"){echo "1";}else{echo "0";}?>">
    <div class="text-start me-2">
      <h6><?php echo $item->ProductName;  if(!empty($item->itemnotes)){ echo " -".$item->itemnotes;}?></h6>
      <select name="varientinfo" class="form-select ps-3 py-2 rounded-0" style="width:120px;" required id="varientinfo">
        <?php foreach($varientlist as $thisvarient){?>
        <option value="<?php echo $thisvarient->variantid;?>" data-title="<?php echo $thisvarient->variantName;?>" data-price="<?php echo numbershow($thisvarient->price, $storeinfo->showdecimal, '.', ''); ?>" <?php if($item->variantid==$thisvarient->variantid){ echo "selected";}?>><?php echo $thisvarient->variantName;?></option>
        <?php }?>
      </select>
    </div>
    <div class="text-center">
      <div class="border cart_counter d-flex justify-content-end p-1 radius-30">
        <button onClick="var result = document.getElementById('sst61_<?php echo $type;?>'); var sst = result.value; if( !isNaN( sst ) &amp;&amp; sst > 0 ) result.value--;return false;" class="bg-green border-0 items-count reduced rounded-circle text-white" type="button" style="<?php echo $button_and_bg_color_css;?>"> <i class="ti-minus"></i> </button>
        <input type="text" name="itemqty" id="sst61_<?php echo $type;?>" maxlength="12" value="1" title="Quantity:" class="border-0 input-text qty text-center width_40">
        <button onClick="var result = document.getElementById('sst61_<?php echo $type;?>'); var sst = result.value; if( !isNaN( sst )) result.value++;return false;" class="bg-green border-0 increase items-count rounded-circle text-white" type="button" style="<?php echo $button_and_bg_color_css;?>"> <i class="ti-plus"></i> </button>
      </div>
      <div class="mt-2" id="vprice"><?php if ($this->storecurrency->position == 1) { echo $this->storecurrency->curr_icon; } echo numbershow($item->price, $storeinfo->showdecimal); if ($this->storecurrency->position == 2) { echo $this->storecurrency->curr_icon;}?></div>
    </div>
  </div>
  ...
  <div class="bg-light p-4"> Addons Information </div>
  <div class="p-4">
    <?php $k=0;
    ...
    <div class="align-items-center border-bottom d-flex justify-content-between mb-3 pb-2">
      ...
      <div><?php if ($this->storecurrency->position == 1) { echo $this->storecurrency->curr_icon; } echo numbershow($addons->price, $storeinfo->showdecimal); if ($this->storecurrency->position == 2) { echo $this->storecurrency->curr_icon;}?></div>
      <div class="border cart_counter d-flex justify-content-end p-1 radius-30">
        <button onClick="var result = document.getElementById('addonqty_<?php echo $addons->add_on_id;?>'); var sst2 = result.value; if( !isNaN( sst2 ) &amp;&amp; sst2 > 0 ) result.value--;return false;" class="bg-green border-0 items-count reduced rounded-circle text-white" type="button" style="<?php echo $button_and_bg_color_css;?>"> <i class="ti-minus"></i> </button>
        ...
        <button onClick="var result = document.getElementById('addonqty_<?php echo $addons->add_on_id;?>'); var sst2 = result.value; if( !isNaN( sst2 )) result.value++;return false;" class="bg-green border-0 increase items-count rounded-circle text-white" type="button" style="<?php echo $button_and_bg_color_css;?>"> <i class="ti-plus"></i> </button>
      </div>
    </div>
    <?php } ?>
  </div>
...
<div class="modal-footer">
	<button type="button" class="btn btn-sm bg-green text-white px-3 py-2" onclick="addonsfoodtocartmulti(<?php echo $item->ProductsID;?>,1,'<?php echo $flag;?>')" style="<?php echo $button_and_bg_color_css;?>"><?php echo display('add_to_cart_more')?></button>
  <button type="button" class="btn btn-sm btn-dark px-3 py-2" onclick="addonsfoodtocart(<?php echo $item->ProductsID;?>,1,'<?php echo $flag;?>')" style="<?php echo $button_and_bg_color_css;?>"><?php echo display('add_to_cart')?></button>
