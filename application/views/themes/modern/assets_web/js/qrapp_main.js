      "use strict";
		...
		var mainqrid=$("#mainqrid").val();
		var name = $('#varientinfo option:selected').data('title');
		var price = $('#varientinfo option:selected').data('price');
		
		$("#sizeid_1"+mainqrid).val(id);
		$("#size_1"+mainqrid).val(name);
		$("#sizeid_1"+mainqrid).val(id);
		$("#size_1"+mainqrid).val(name);
		$("#varient_1"+mainqrid).val(name);
		$("#itemprice_1"+mainqrid).val(price);
		$("#itemprice_1"+mainqrid).val(price);
		$("#vprice").text(price);
	});
        function scrollNav() {
		...
		$(".withaddons").click(function() {
            $(this).removeClass("d-none");
            $(this).siblings(".cart_counter").removeClass("active");
        ...
        function itemreduce(pid, vid, id, event = null) {
            if (event) {
                event.preventDefault(); // Prevent the click from propagating to parent elements
             }
        ...
        function itemincrese(pid, vid, id, event = null) {
            if (event) {
        ...
        function getfoodlist(id) {
            ...
            var dataString = "foodname=" + foodname+'&cateid='+id+'&csrf_test_name='+basicinfo.csrftokeng;
            $.ajax({
                type: "POST",
