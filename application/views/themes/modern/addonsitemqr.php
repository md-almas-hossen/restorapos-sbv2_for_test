...
<div class="bg-primary modal-header" style="min-width: 2.5rem; background:<?php echo !empty($qrtheme->qrbuttoncolor) ? $qrtheme->qrbuttoncolor . '!important' : '#940753 !important'; ?>; color:<?php echo !empty($qrtheme->qrbuttontextcolor) ? $qrtheme->qrbuttontextcolor : '#ffffff'; ?>;">
          <h1 class="modal-title fs-5 text-white" id="itemDetailsLabel"><?php echo display('food_details')?></h1>
...
<input name="mainqrid" type="hidden" id="mainqrid" value="<?php echo $type;?>" />  
          <div class="row mb-3">
            <div class="col-7">
                ...
                <input name="itemprice" type="hidden" value="<?php echo $item->price;?>" id="itemprice_1<?php echo $type;?>" />
              ...
              <select name="varientinfo" class="form-select ps-3 py-2 rounded-0" required id="varientinfo">
        ...
        <?php }?>
            ...
            <div class="col-5 align-self-end">
              ...
              <div class="mt-1 text-center" id="vprice"><?php echo $item->price;?></div>
            </div>
          </div>
...
<div class="form-check mb-1 d-flex justify-content-between">
  <div>
    ...
    <label class="form-check-label d-flex justify-content-between" for="addons_<?php echo $addons->add_on_id;?>">
      ...
      <span class="ms-1"><?php echo $addons->price;?></span>
    </label>
  </div>
 ...
 <?php } }?>
        ...
        <?php if($type=="menu"){$flag="menu";}else{ $flag="other";} ?>
          <button type="button" onclick="addonsfoodtocartmulti(<?php echo $item->ProductsID;?>,1,'<?php echo $flag;?>')" class="btn btn-primary" style="background:<?php echo !empty($qrtheme->qrbuttonhovercolor) ? $qrtheme->qrbuttonhovercolor : '#85054a'; ?>; color:<?php echo !empty($qrtheme->qrbuttontextcolor) ? $qrtheme->qrbuttontextcolor : '#ffffff'; ?>;"><?php echo "Cart & More";?></button>
          <button type="button" class="btn btn-secondary" onclick="addonsfoodtocart(<?php echo $item->ProductsID;?>,1,'<?php echo $flag;?>')" style="background:<?php echo !empty($qrtheme->qrbuttoncolor) ? $qrtheme->qrbuttoncolor : '#940753'; ?>; color:<?php echo !empty($qrtheme->qrbuttontextcolor) ? $qrtheme->qrbuttontextcolor : '#ffffff'; ?>;"><?php echo display('add_to_cart') ?></button>
        </div>
...
$(".btn_push").click(function() {
		...
		if($('.btn_push').is(':checked') ){
    		var pos=$(this).val();
			var id=$(this).attr('lang');
			$("#topping_"+id).attr('pos',pos);
		}
		else{
