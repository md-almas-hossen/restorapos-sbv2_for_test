...
class Supplier_model extends CI_Model {
    ...
    $coainfo = $this->db->select('*')->from('acc_subcode')->where('HeadName',$c_acc)->get()->row();
...
public function countlist()
	...
	public function supplier_ledger_report($supplierid= null,$fromdate= null,$todate= null,$limit = null, $start = null){
			...
			if(!empty($supplierid)){
			$this->db->where('purchaseitem.suplierID', $supplierid);
			$this->db->where(array('purchaseitem.purchasedate >=' => $fromdate, 'purchaseitem.purchasedate <=' => $todate));
			}
			//$this->db->limit($limit, $start);
			$query = $this->db->get();
			if ($query->num_rows() > 0) {
