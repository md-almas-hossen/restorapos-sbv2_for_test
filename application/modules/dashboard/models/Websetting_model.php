...
class Websetting_model extends CI_Model {
 ...
 //menu section
	...
	public function createnewzone($data = array())
  {   
    return $this->db->insert('tbl_location_zone',$data);
  }
  
  public function updatezone($data = array())
  {  
    return $this->db->where('id',$data["id"])
      ->update('tbl_location_zone', $data);
  }

  public function deletezone($id = null)
  {
    $this->db->where('id',$id)->delete('tbl_location_zone');
  ...
  public function read_color_setting()
  {
	  return $this->db->select("*")
		  ->from('color_setting')
		  ->get()
