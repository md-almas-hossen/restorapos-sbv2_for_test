...
class Web_setting extends MX_Controller {
	...
	public function common_create()
	{
		...
		$data['setting'] = (object)$postData = array(
			...
			'ismapenable'        => $this->input->post('mapenable', true),
			'mapapikey'  		 => $this->input->post('mapkey', true),
			'quick_checkout'  	 => $this->input->post('quick_checkout', true),
		); 
		#-------------------------------#
...
public function delivaryaddress()
  {
    ...
    $data['module']   = "dashboard";  
    $data['address_list'] = $this->db->select('*')->from('tbl_delivaryaddress')->join('tbl_location_zone','tbl_location_zone.id=tbl_delivaryaddress.zone_id')->order_by('tbl_delivaryaddress.delivaryid', 'DESC')->get()->result(); 
    $data['zone_list'] = $this->db->select('*')->from('tbl_location_zone')->get()->result(); 
...
public function getZonePrice(){
    $id=$this->input->post('id',true);
     $price=$this->db->select('*')->from('tbl_location_zone')->where('id',$id)->get()->row(); 
...
public function delivarityme()
	...
	//Zone setting
	public function location_zone()
  {
    $data['title'] = display('location_zone');
    $data['module']   = "dashboard";  
    $data['zone_list'] = $this->db->select('*')->from('tbl_location_zone')->get()->result(); 
    $data['page']   = "web/location_zone";  
    echo Modules::run('template/layout', $data); 
  }

  public function createzone(){
    $data['title'] = display('create_address');
    $this->form_validation->set_rules('zone_name',display('location_zone'),'required');
    ...
    $postData = array(
      ...
      if ($this->form_validation->run() === true) {
          if ($this->websetting_model->createnewzone($postData)) {
            #set success message
            $this->session->set_flashdata('message',display('save_successfully'));
          } else {
            #set exception message
        ...
        redirect('dashboard/web_setting/location_zone');
  ...
  public function editzone($id){
    $data['title'] = display('delv_address');
      ...
      if ($this->form_validation->run() === true) {
          if ($this->websetting_model->updatezone($postData)) {
            #set success message
            $this->session->set_flashdata('message',display('update_successfully'));
          } else {
            #set exception message
  ...
  public function deletezone($menuid = null)
    {
    if ($this->websetting_model->deletezone($menuid)) {
      #set success message
      $this->session->set_flashdata('message',display('delete_successfully'));
    } else {
      #set exception message
	...
	// Coloe Setting for website part
	public function color_setting()
	{
		$data['title'] = display('color_setting');
		#-------------------------------#
		//check setting table row if not exists then insert a row
		#-------------------------------#
		$data['color_setting'] = $this->websetting_model->read_color_setting(); 
		// dd($data);

		$data['module'] = "dashboard";  
		$data['page']   = "web/color_setting";  
		echo Modules::run('template/layout', $data); 
	}

	public function update_color_setting(){
        $web_text_color       = $this->input->post('web_text_color');
        $web_button_color     = $this->input->post('web_button_color');
        $web_header_bg_color  = $this->input->post('web_header_bg_color');
        $web_footer_bg_color  = $this->input->post('web_footer_bg_color');
        $id                   = ($this->input->post('id')?$this->input->post('id'):'');

        $data = array(
            'id'                    => $id,
            'web_text_color'        => $web_text_color,
            'web_button_color'      => $web_button_color,
            'web_header_bg_color'   => $web_header_bg_color,
			'web_footer_bg_color'   => $web_footer_bg_color,
        );
      
		$this->db->select('*');
        $this->db->from('color_setting');
        $check = $this->db->get()->num_rows();
        
        if($check > 0){

			$udata = $this->db->where('id',$data['id'])
			->update('color_setting',$data); 

        }else{
            
            $add_setting = $this->db->insert('color_setting',$data);
        }

		$this->session->set_flashdata('message',display('update_successfully'));
        redirect('dashboard/web_setting/color_setting');
    }
}
