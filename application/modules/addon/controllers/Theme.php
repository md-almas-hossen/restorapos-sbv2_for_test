...
class Theme extends MX_Controller {
  ...
  private $theme_location = APPPATH.'views/themes/';
    ...
    public function active_theme($name)
    {
		...
		$getname=rawurldecode($name);
		$getname2=substr($getname, 3, -4);
		$newname=base64_decode($getname2);
		//if($newname!="modern"){
		$this->db->set('status', 1)->where('themename', $newname)->update('themes');
        $this->db->set('status', 0)->where('themename !=', $newname)->update('themes');
		//}
