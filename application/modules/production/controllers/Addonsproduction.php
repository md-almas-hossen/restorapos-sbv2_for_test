...
class Addonsproduction extends MX_Controller {
    ...
    public function index($id = null)
    {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('production/addonsproduction/index');
        $config["total_rows"]  = $this->addonsproduction_model->countlist();
        $config["per_page"]    = 25;
        ...
        $data["productionlist"] = $this->addonsproduction_model->read($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
		$data['pagenum']=$page;
		$settinginfo=$this->addonsproduction_model->settinginfo();
		$data['currency']=$this->addonsproduction_model->currencysetting($settinginfo->currency);
        #
        #pagination ends
        #   
	...
	public function productionunit(){
	  ...
	  #-------------------------------#
	   ...
	   $data['intinfo']="";
	   $data['item']   = $this->addonsproduction_model->item_dropdown();
	   $data['ingrdientslist']   = $this->addonsproduction_model->ingrediantlist();
	   $data['module'] = "production";
	   $data['page']   = "addproductionaddons";   
	   echo Modules::run('template/layout', $data); 
		}
	public function production_entry(){
		$this->permission->method('production','create')->redirect();
		$this->form_validation->set_rules('add_on_id','Addons Name','required');
	    $saveid=$this->session->userdata('id'); 
		
	   if ($this->form_validation->run()) { 
		$this->permission->method('production','create')->redirect();
		 $logData = [
		   'action_page'         => "set Addons Production Unit",
		   'action_done'     	 => "Insert Data", 
		   'remarks'             => "set Addons Production Unit Created",
		   'user_name'           => $this->session->userdata('fullname'),
		   'entry_date'          => date('Y-m-d H:i:s'),
		  ];
		if ($this->addonsproduction_model->makeproduction()) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('production/addonsproduction/addproduction');
		} 
		else{
	...
	public function insert_production(){
			...
			if ($this->addonsproduction_model->create()) { 
			 $this->logs_model->log_recorded($logData);
			 $this->session->set_flashdata('message', display('save_successfully'));
			 redirect('production/addonsproduction/create');
			} 
			else{
	...
	public function productionitem(){
           ...
           $product_info 	= $this->addonsproduction_model->finditem($product_name);
	...
	public function getfoodfarient($id){
			$varientlist=$this->addonsproduction_model->foodvarientlist($id);
	...
	public function productionquantity(){
		...
		$product_info =  $this->addonsproduction_model->get_total_product($product_id);
   ...
   public function updateintfrm($id,$vid){
		...
		$data['item']   = $this->addonsproduction_model->item_dropdown();
		$data['ingrdientslist']   = $this->addonsproduction_model->ingrediantlist();
		$data['productioninfo']   = $this->addonsproduction_model->findById($id,$vid);
		$data['varientinfo']   = $this->addonsproduction_model->findByvId($id);
		$data['iteminfo']       = $this->addonsproduction_model->iteminfo($id,$vid);
        $data['module'] = "production";  
 	...
 	public function update_entry(){
		...
		if ($this->addonsproduction_model->update()) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('update_successfully'));
		 redirect('production/addonsproduction/index');
		} else {
		 $this->session->set_flashdata('message',  display('update_successfully'));
		}
		redirect("production/addonsproduction/index");
	...
	public function getset($id,$vid){
		...
		$data['item']   = $this->addonsproduction_model->item_dropdown();
		$data['ingrdientslist']   = $this->addonsproduction_model->ingrediantlist();
		$data['productioninfo']   = $this->addonsproduction_model->readset($id,$vid);
		$data['iteminfo']       = $this->addonsproduction_model->iteminfo($id,$vid);
        $data['module'] = "production";  
  ...
  public function getlist($id){
	 	 $suplierinfo=$this->addonsproduction_model->suplierinfo($id);
    ...
    public function delete($id = null)
    {
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		if ($this->addonsproduction_model->deleteitem($productid,$pdetailid)) {
	...
	public function ingredientcheck(){
		...
		echo $available=$this->addonsproduction_model->checkingredientstock($foodid,$vid,$qty);
		}
