...
class Production_model extends CI_Model {
	...
	{
	    $this->db->select('production_details.foodid,production_details.	receipe_code,production_details.receipe_price,production_details.pvarientid,item_foods.ProductName,variant.variantName,variant.variantid');
        $this->db->from('production_details');
		...
		FROM
			(
				...
				WHERE
					...
					AND `ingredients`.`ingredient_name` LIKE '%$product_name%' ESCAPE '!'
				GROUP BY
					`ingredients`.`id`
...
public function iteminfo($id,$vid=null){
		...
		if ($query->num_rows() > 0) {
			...
			foreach ($results as $result) {
				...
				$this->db->select('ingredients.*,SUM(purchase_details.quantity) as uquantity,SUM(purchase_details.totalprice) as uprice');
				$this->db->from('ingredients');
				$this->db->join('purchase_details','purchase_details.indredientid = ingredients.id','inner');
				$this->db->where('ingredients.is_active',1);
				$this->db->where('ingredients.ingredient_name', $result->ingredient_name);
				$this->db->group_by('ingredients.id');
				$query = $this->db->get();
				if ($query->num_rows() > 0) {
					$value = $query->row();
				}
				$this->db->select('ingredients.*,SUM(tbl_openingstock.openstock) as uquantity,SUM(tbl_openingstock.unitprice*tbl_openingstock.openstock) as uprice');
				$this->db->from('ingredients');
				$this->db->join('tbl_openingstock','tbl_openingstock.itemid = ingredients.id','inner');
				$this->db->where('tbl_openingstock.itemtype',0);
				$this->db->where('ingredients.is_active',1);
				...
				$results[$i]->uprice=$value->uprice/$value->uquantity;
				$i++;
			}
			return $results;	
		}
...
#check stock
	public function checkingredientstock($foodid,$vid,$foodqty){
		...
		//print_r($productionset->ingredientid);
		/*
		SELECT itemid, sum(menuqty*d.qty) as sale_qty
    FROM production p 
    left join production_details d on p.receipe_code = d.receipe_code    
    left join (  
	    SELECT menu_id,sum(`menuqty`) menuqty 
        FROM `order_menu` 
        WHERE menu_id  = 1
        group by menu_id
    ) s on s.menu_id = p.itemid
    Group by itemid */
