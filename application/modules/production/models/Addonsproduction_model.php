...
class Addonsproduction_model extends CI_Model {
	
	private $table = 'addonsproduction_details';
	...
	public function create()
	{
		...
		$this->db->insert('addonsproduction',$data);
	...
	public function checkproductiondetails($foodid,$fvid,$foodqty)
	{
		...
		if(!empty($checksetitem)){
			...
			foreach($groupitemlist as $groupitem){
				$this->db->select('*');
				$this->db->from('addonsproduction_details');
	...
	public function update()
	{
		...
		for ($i=0, $n=count($p_id); $i < $n; $i++) {
			...
			$product_id = $p_id[$i];
				...
				if(!empty($quantity))
				{
					$this->db->insert('addonsproduction_details',$data1);
				}
	...
	public function makeproduction()
	{
		$saveid=$this->session->userdata('id');
		$p_id = $this->input->post('product_id');
		$itemid=$this->input->post('add_on_id');
		$quantity = $this->input->post('product_quantity',true);
		$newdate= date('Y-m-d');
		$this->db->select('*');
		$this->db->from('addonsproduction_details');
		$this->db->where('addonsid',$itemid);
		$query = $this->db->get();
		if ($query->num_rows() > 0){
			return false;
		}
	...
	{ 
		return $this->db->select("*")->from('addonsproduction_details')
			->where('foodid',$id) 
...
//item Dropdown*/
 ...
 public function item_dropdown()
	{
		$data = $this->db->select("add_ons.*,menu_add_on.row_id,menu_add_on.menu_id")
			->from('add_ons')
			->join('menu_add_on','menu_add_on.add_on_id=add_ons.add_on_id','inner')
			->where('menu_add_on.is_active',1)
			->get()
			->result();
