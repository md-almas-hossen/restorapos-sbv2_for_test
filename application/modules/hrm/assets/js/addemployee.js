...
function valid_inf6() {
	...
	var homeemail = $('#h_email').val();
	var businessemail = $('#b_email').val();
	/*if(isEmail(homeemail)){
...
$(document).ready(function() {
	var countryinfo=$("#country").val();
	var csrf = $('#csrfhashresarvation').val();
    var myurl = basicinfo.baseurl+'hrm/Employees/statelist';
    var dataString = "country=" + countryinfo+'&csrf_test_name='+csrf;
    $.ajax({
        type: "POST",
        url: myurl,
        data: dataString,
...
function checkexitemail(email,id,empid=null){
			var csrf = $('#csrfhashresarvation').val();
			var myurl = basicinfo.baseurl+'hrm/Employees/checkemail';
			var dataString = "email=" + email+'&empid='+empid+'&csrf_test_name='+csrf;
			$.ajax({
				type: "POST",
				url: myurl,
				data: dataString,
    ...
    $('#hidepasstext').click(function() {
        $('#hidepasstext').removeClass("show");
        $('#hidepasstext').addClass("hide");
        $('#showpasstext').removeClass("hide");
        $('#showpasstext').addClass("show");
        $("#password").attr('type',"text")
    });
    $('#showpasstext').click(function() {
        $('#hidepasstext').removeClass("hide");
        $('#hidepasstext').addClass("show");
        $('#showpasstext').removeClass("show");
        $('#showpasstext').addClass("hide");
