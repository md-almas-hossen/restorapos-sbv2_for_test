...
public function delete_salsetup($id = null)
	...
	{
		return $this->db->where('salary_type_id', $data["salary_type_id"])->update("salary_type", $data);
	}
...
//monthly deduct
	...
	{
	 return $this->db->select('tbl_monthly_deduct.*,salary_type.sal_name,employee_history.first_name,employee_history.last_name')
	->from('tbl_monthly_deduct')
	->join('salary_type','salary_type.salary_type_id=tbl_monthly_deduct.duductheadid','left')
	->join('employee_history','employee_history.employee_id=tbl_monthly_deduct.employee_id','left')
	->get()->result();
...
/* Salary Setup start ****/
	...
	public function salary_gen_delete($id = null,$month=null)
	{
		
		$this->db->where('name',$month)->delete('salary_sheet_generate');
		if ($this->db->affected_rows()) {
			$this->db->where('paymentgeneratedate',$month)->delete('employee_salary_payment');
			return true;
		} else {
			return false;
		}
	...
	{
		return $this->db->where('ssg_id', $data["ssg_id"])->update("salary_sheet_generate", $data);
	}
...
/* salary setup Update ********************************************/
    ...
    public function salary_amount($id)
	{
		return $result = $this->db->select('employee_salary_setup.*,salary_type.*')	
			 ...
			 ->join('salary_type','salary_type.salary_type_id=employee_salary_setup.salary_type_id','left')
...
public function employee_salary_charts($ssg_id)
	{
		  ...
		  //return $salary_sheet_generate_info; GROUP_CONCAT(CONCAT('''', employee_id, '''' ))
			...
			->where('pment.paymentgeneratedate',$salary_sheet_generate_info->name)
            ->group_by('pment.emp_sal_pay_id')
            ->order_by('pment.emp_sal_pay_id', 'desc')
            ->get()
            ->result();
	//return $this->db->select("GROUP_CONCAT(CONCAT('''', employee_id, '''' )) as empid")->from('salary_sheet_generate')->where('name',$salary_sheet_generate_info->name)->get()->row();
	}
