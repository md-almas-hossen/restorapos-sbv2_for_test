...
class Employees_model extends CI_Model
{
	...
	{
		return $this->db->where('emp_sal_set_id', $data["emp_sal_set_id"])->update("employee_salary_setup", $data);
	}
	...
	public function emp_paymentView($empid = null, $month = null)
	{
		$this->db->select('count(DISTINCT(pment.emp_sal_pay_id)) as emp_sal_pay_id,pment.*,p.employee_id,p.first_name,p.last_name');
		$this->db->from('employee_salary_payment pment');
		$this->db->join('employee_history p', 'pment.employee_id = p.employee_id', 'left');
		if (!empty($empid)) {
			$this->db->where('pment.employee_id', $empid);
		}
		if (!empty($month)) {
			$this->db->where('pment.paymentgeneratedate', $month);
		}
		$this->db->group_by('pment.emp_sal_pay_id');
	...
	// modified code by mk starts here...
	public function emp_list()
	{
		 if ($this->db->table_exists('real_ip_setting')){
		// starts here
		$this->db->select('p.*,d.department_name,po.position_name,dt.type_name,rt.r_type_name as rd_type,gd.gender_name,ms.marital_sta,pf.frequency_name, dip.device_name, dip.device_ip');
		...
		// starts here
		$this->db->join('tbl_device_ip dip', 'p.device_ip_id = dip.id', 'left');
		// ends here
		}else{
		 $this->db->select('p.*,d.department_name,po.position_name,dt.type_name,rt.r_type_name as rd_type,gd.gender_name,ms.marital_sta,pf.frequency_name');
		// $this->db->select('p.*,d.department_name,po.position_name,dt.type_name,rt.r_type_name as rd_type,gd.gender_name,ms.marital_sta,pf.frequency_name');
