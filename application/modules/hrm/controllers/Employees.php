...
/* ################ Employee Performance Start   #######################....*/
	...
	{
		$this->permission->module('hrm', 'read')->redirect();
		$employee = $this->input->post('employeeid', true);
		$month = $this->input->post('month', true);

		$data['title']         = display('view_employee_payment');
		$data['employeelist']       = $this->Employees_model->emp_historyview();
		$data['emp_pay']       = $this->Employees_model->emp_paymentView($employee, $month);
		$data['module']        = "hrm";
	...
	public function getsalaryslip()
	{
		...
		$data['emp_pay']       = $this->Employees_model->emp_paymentView($employee, $month);
		$data['module']        = "hrm";
		$data['page']          = "paymentviewajax";
		$this->load->view('paymentviewajax', $data);
	}
	...
	public function viewEmhistory()
	{
		...
		$data['device_ip_dropdown'] = $this->Employees_model->device_ip();
		}
	...
	public function create_employee()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if ($this->db->table_exists('real_ip_setting')){
				$postData['device_ip_id']=$this->input->post('device_ip_id', true);
				}
			...
			if ($this->db->insert('employee_history', $postData)) {
				...
				//$acc_subtype=$this->db->select("*")->from('acc_subtype')->where('name','Employee')->get()->row();
				$postData1 = array(
					'name'         	=> $this->input->post('first_name', true), //$this->input->post('customer_name',true),
					'subTypeID'        => 2, //$acc_subtype->code,
					'refCode'          => $empid //$rerturnid						 
				);
	...
	public function update_employee_form($id = null)
	{
		...
		if ($this->form_validation->run() === true) {
			...
			if ($this->Employees_model->update_employee($postData)) {
				...
				$tblsubtype = $this->db->select("*")->from('acc_subtype')->where('name', 'Employee')->get()->row();
				$subcodeinfo = $this->db->select("*")->from('acc_subcode')->where('subTypeID', $tblsubtype->code)->where('refCode', $empid->emp_his_id)->get()->row();
				if ($subcodeinfo) {
					$acc = array(
						'name'  => $this->input->post('first_name', true),
					);
					$this->db->where('refCode', $empid->emp_his_id);
			...
			// new code by mk starts here. Update by Ainal
			if ($this->db->table_exists('real_ip_setting')){
		  	$data['device_ip_dropdown'] = $this->Employees_model->device_ip();
			}
			// new code by mk ends here
	...
	public function payconfirm($id = null)
	{
		...
		$employeeinfo = $this->db->select('emp_his_id')->from('employee_history')->where("employee_id", $this->input->post('employee_id', true))->get()->row();
		$subcodeinfo = $this->db->select('id,refCode')->from('acc_subcode')->where("refCode", $subcode)->get()->row();
		$headcode = 22;
		$subtype = 2;
		$subcode = $subcodeinfo->id;
		$comments = "Pay to Employee";
		$amount = $this->input->post('total_salary', true);

		$CreateBy = $this->session->userdata('fullname');
		$createdate = date('Y-m-d H:i:s');
		$financialyears = $this->db->select('*')->from('tbl_financialyear')->where("is_active", 2)->get()->row();

		$employee_id = $subcodeinfo->refCode;
		$cinsert = array(
			'Vno'            =>  $voucher_no,
			'Vdate'          =>  $VDate,
			'companyid'      =>  0,
		...
		$debitinsert = array(
			'voucherheadid'     =>  $insert_id,
			'HeadCode'          =>  $headcode,
			'Debit'          	  =>  $amount,
			'Creadit'           =>  0,
			'RevarseCode'       =>  $reversecode,
			'subtypeID'         =>  $subtype,
			'subCode'           =>  $subcode,
			'LaserComments'     =>  $comments,
			'chequeno'          =>  $checkno,
			'chequeDate'        =>  $checkdate,
			'ishonour'          =>  $ishonours
		);
		$this->db->insert('tbl_vouchar', $debitinsert);
		...
		$this->db->insert('acc_transaction', $payemployee);
		//echo $this->db->last_query();
		//Employee For Credit
		$payemployeerev  = array(
			'VNo'            => $insert_id,
			'Vtype'          => 1,
			'VDate'          => $VDate,
			'COAID'          => $reversecode,
		...
		//echo $this->db->last_query();
		$this->db->insert('acc_transaction', $payemployeerev);
		if ($this->Employees_model->update_payment($postData)) {
	...
	public function checkemail()
	{
		...
		$userinfo = $this->db->select('id,email')->from('user')->where('email', $email)->get()->row();
		if (!empty($userinfo)) {
			if ($userinfo->id == $empid) {
				echo 0;
			} else {
				echo 1;
			}
