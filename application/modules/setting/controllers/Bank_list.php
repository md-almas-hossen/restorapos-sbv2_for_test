...
class Bank_list extends MX_Controller {
    ...
    public function index($id = null)
    {
		...
		$data['coas'] = $this->db->select('id,account_name')->from('acc_coas')->where('head_level', 4)->where('is_bank_nature',1)->where('is_active',1)->get()->result();
  ...
  public function setdefault(){
	  		...
	  		$resetupdate=array('setdefault'=>0);
			$this->db->update('tbl_bank', $resetupdate);
			
			$updata=array('setdefault'=>1);
			$this->db->where('bankid',$cardid)->update('tbl_bank', $updata);
	  }
	...
	public function updatestatus(){
		...
		$bank_info = $this->db->select("*")
			->from('tbl_bank')
			->where('bankid', $cardid)
			->get()
			->row();
		
		if(isset($bank_info->is_active) && $bank_info->is_active == 1){
			
			$updata=array('is_active'=>0);
			$res_up = $this->db->where('bankid',$cardid)->update('tbl_bank', $updata);

			if($res_up){
				echo 2;exit; // Means Activated
			}else{
				echo 0;exit;
			}

		}else if(isset($bank_info->is_active) && $bank_info->is_active == 0){

			$updata=array('is_active'=>1);
			$res_up = $this->db->where('bankid',$cardid)->update('tbl_bank', $updata);

			if($res_up){
				echo 1;exit; // Means Deactivated
			}else{
				echo 0;exit;
			}
		}else{
