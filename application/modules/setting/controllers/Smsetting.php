...
class Smsetting extends MX_Controller {
    ...
    public function sms_configuration(){
		...
		$data['globalsetting'] =$this->db->select('issmsenable')->from('setting')->where('id',2)->get()->row();
		$data['module'] = "setting";  
		$data['page']   = "sms/form"; 
		echo Modules::run('template/layout', $data);
    }
	
	public function sms_enable(){
			$data=array(    					
    		'issmsenable' => $this->input->post('status'),
    		);
			$resutl=$this->db->where('id',2)->update('setting',$data);
		return $result;
		}
