...
class Currency extends MX_Controller {
   ...
   public function updateintfrm($id){
	...
	public function currency_notes($id = null){
        
		$this->permission->method('setting','read')->redirect();
        $data['title']    = display('currency_note_list'); 
        #-------------------------------#       
        #
        #pagination starts
        #
        $config["base_url"] = base_url('setting/currency/currency_notes');
        $config["total_rows"]  = $this->currency_model->currencynotescountlist();
        $config["per_page"]    = 25;
        $config["uri_segment"] = 4;
        $config["last_link"] = "Last"; 
        $config["first_link"] = "First"; 
        ...
        $this->pagination->initialize($config);
        $page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        $data["currencynotelist"] = $this->currency_model->currencynotes_read($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
        ...
        #   
        $data['module'] = "setting";
        $data['page']   = "currency_notelist";   
        echo Modules::run('template/layout', $data); 
    }

	public function save_currencynote($id = null){
	  $this->permission->method('setting','create')->redirect();
	  $data['title'] = display('add_currencynote');
	  #-------------------------------#
	   ...
	   $saveid=$this->session->userdata('id');
	   $data['type']   = (Object) $postData = array(
		   'id'  		    => $this->input->post('currencynoteid'),
		   'title'  		    => $this->input->post('title'),
		   'amount' 			=> $this->input->post('amount',true),
		   'orderpos' 			=> $this->input->post('orderno',true),
		   'created_date' 		=> date('Y-m-d H:i:s'),
		  ); 
	  $data['intinfo']="";
	  if ($this->form_validation->run()) { 
	   if(empty($this->input->post('currencynoteid'))) {
		
		$this->permission->method('setting','create')->redirect();
	 $logData =array(
	   'action_page'         => "Currency Note List",
	   'action_done'     	 => "Insert Data", 
	   'remarks'             => "New Currency Note Created",
	   'user_name'           => $this->session->userdata('fullname'),
	   'entry_date'          => date('Y-m-d H:i:s'),
	  );
		if ($this->currency_model->currencynotecreate($postData)) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('save_successfully'));
		 redirect('setting/currency/currency_notes');
		} else {
		 $this->session->set_flashdata('exception',  display('please_try_again'));
		}
		redirect("setting/currency/currency_notes"); 
	  ...
	  $logData = array(
	   'action_page'         => "Currency Note List",
	   'action_done'     	 => "Update Data", 
	   'remarks'             => "Currency Note Updated",
	   'user_name'           => $this->session->userdata('fullname'),
	   'entry_date'          => date('Y-m-d H:i:s'),
	  );

	  
		if ($this->currency_model->currencynote_update($postData)) { 
		 $this->logs_model->log_recorded($logData);
		 $this->session->set_flashdata('message', display('update_successfully'));
		} else {
		$this->session->set_flashdata('exception',  display('please_try_again'));
	   ...
	   if(!empty($id)) {
		$data['title'] = display('currency_note_edit');
		$data['intinfo']   = $this->currency_model->currencynotefindById($id);
	   }
	...
	public function noteupdatefrm($id){
		$this->permission->method('setting','update')->redirect();
		$data['title'] = display('currency_note_edit');
		$data['intinfo']   = $this->currency_model->currencynotefindById($id);
		
        $data['module'] = "setting";  
        $data['page']   = "currency_noteedit";
		$this->load->view('setting/currency_noteedit', $data);   
    ...
    public function note_delete($id = null){
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		if ($this->currency_model->note_delete($id)) {
			#Store data to log table.
			 $this->logs_model->log_recorded($logData);
			#set success message
			$this->session->set_flashdata('message',display('delete_successfully'));
	...
	public function get_currencyNoteAmount(){
		$noteid = $this->input->post('noteid');
		$get_currencyNoteAmount = $this->currency_model->currencynotefindById($noteid);
		
		$currency_notesdata = array(
			'amount' => $get_currencyNoteAmount->amount,
		);
		echo json_encode($currency_notesdata);
	}
