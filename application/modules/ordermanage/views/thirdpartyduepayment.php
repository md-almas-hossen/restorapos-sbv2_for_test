...
<?php
echo form_open('ordermanage/order/thirdparty_duepayment_save', 'method="post" class="navbar-search" name="" id="paymodal-multiple-form"') ?>
        ...
        <table class="table table-fixed table-bordered table-hover bg-white" style="width:100%;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">
                            <span class="single-order-com-<?php echo $sl; ?>">
                                <?php echo ($order->commission_amount); ?>
                            </span>
                        <!-- <input type="hidden" name="commission_percentage[]" value="<?php //echo ($order->commission_percentage); ?>"> -->
                        <input type="hidden" class="allcom commission-check-value-<?php echo $sl; ?> commission-amount"
                            value="<?php echo ($order->commission_amount); ?>" name="commission_amount[]">
                        <?php
                ...
                <tr>
                    ...
                    <?php //if($total_commission > 0):?>
                        <td class="text-right total-single-order-commission"><?php echo $total_commission; ?></td>
                    <?php //endif;?>
                </tr>
            </tbody>
        </table>
    ...
    <div class="col-sm-6">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-4">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control text-right" id="paid_amount" name="paid_amount" required>
                </div>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-4">
                <label for="payment_method_id" class="col-sm-6 col-form-label">Commission</label>
                <div class="col-sm-6">
                   <input type="text" class="form-control text-right" id="total_commission" value="<?php echo $total_commission;?>" readonly>
                </div>
            </div>
        </div>
        <?php endif;?>
...
$("body").ready(function() {
    ...
    $(".commission-amount").each(function() {
        com += parseFloat(this.value);
    });   

    // $('#total_amount').val((sum-com).toFixed(2));
    ...
    // mkar
        ...
        discountPercentage.on('input', function() {
            ...
            // $('#total_amount').val(((sum-com) - ((percentage / 100) * sum)).toFixed(2));
        ...
        discountFlat.on('input', function() {
            var flat = $(this).val();
            // $('#total_amount').val(((sum-com) - flat).toFixed(2));
    ...
    $('body').on('click', '#dueOrderSubmit', function (event) {
        ...
        $('body').on('click','.allorder',function(){
           if($(this).prop("checked") == true){
               ...
               $(".commission-amount").each(function() {
                    i++;
                    var singlecom=parseFloat(this.value);
                    totalcom += parseFloat(this.value);
                    $(".single-order-com-" + i).text(totalcom);
                });   
                
                // $('#total_amount').val((totalamnt-totalcom).toFixed(2));
                $('#total_amount').val(totalamnt.toFixed(2));
                // check
                $('#paid_amount').val(totalamnt.toFixed(2));
                $('#total_commission').val(totalcom.toFixed(2));
				// new
            }else{
...
function orderAmount(sl, order_id) {
    ...
    var single_order_com = $('.commission-check-value-'+sl).val();
    ...
    if ($('.marg-check-' + sl).is(':checked')) {
        ...
        // new
        $(".commission-check-value-" + sl).addClass('commission-amount');
        $(".single-order-com-" + sl).text(single_order_com);
        // new
        ...
        // new
        $(".commission-check-value-" + sl).removeClass('commission-amount');
        $(".single-order-com-" + sl).text('0.00');
    ...
    $(".commission-amount").each(function() {
		//   alert(this.value);
        comsum += parseFloat(this.value);
    });

  
    
    $('.total-single-order-amount').text(sum.toFixed(2));
    $('.total-single-order-commission').text(comsum.toFixed(2));
    $('.total-single-order-dueamount').text(sum.toFixed(2));
    // $('#total_amount').val((sum-comsum).toFixed(2));
    $('#total_amount').val(sum.toFixed(2));
    // check
    $('#paid_amount').val(sum.toFixed(2));
    $('#total_commission').val(comsum.toFixed(2));
    // $('#paid_amount').val(sum.toFixed(2));
...
// mkar
    ...
    discountPercentage.on('input', function() {
        var percentage = $(this).val();
        // $('#total_amount').val(((sum-comsum) - ((percentage / 100) * sum)).toFixed(2));
        $('#total_amount').val((sum - ((percentage / 100) * sum)).toFixed(2));
        // check
        $('#paid_amount').val((sum - ((percentage / 100) * sum)).toFixed(2));
        discountFlat.val('');
    ...
    discountFlat.on('input', function() {
        var flat = $(this).val();
        // $('#total_amount').val(((sum-comsum) - flat).toFixed(2));
        $('#total_amount').val(((sum) - flat).toFixed(2));
        // check
        $('#paid_amount').val(((sum) - flat).toFixed(2));
        discountPercentage.val('');
