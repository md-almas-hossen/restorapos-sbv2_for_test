	                    ...
	                    <div class="panel-body">
	                        <div class="row">
	                            ...
	                            <div class="col-sm-2 text-left mb-display">
	                                ...
	                                <div><?php echo display('invoice_no') ?>: <?php echo getPrefixSetting()->sales. '-'.$orderinfo->saleinvoice;?></div>
	                        ...
	                        <div class="table-responsive m-b-20">
	                            <table class="table table-striped table-bordered table-hover bg-white" id="purchaseTable">
                                <thead>
                                     <tr>
                                ...
                                <?php $i=0; 
								  $totalamount=0;
                                    ...
                                    <tr>
                                    	<td class="text-right" colspan="4"><strong>Adjustment (#<?php echo $billinfo->return_order_id;?>)</strong></td>
                                        <td class="text-right"><strong><?php if($currency->position==1){echo $currency->curr_icon;}?>    <?php echo $billinfo->return_amount;?> <?php if($currency->position==2){echo $currency->curr_icon;}?> </strong></td>
                                    </tr>
                                    <?php } ?>
                                    <tr>
                                    	<td class="text-right" colspan="4"><strong><?php echo display('grand_total')?></strong></td>

										<?php if($isvatinclusive = 1 ):?>
                                        	<td class="text-right"><strong><?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo $gtoal = $itemtotal+$servicecharge-$discount-$billinfo->return_amount;?> <?php if($currency->position==2){echo $currency->curr_icon;}?> </strong></td>
										<?php else:?>
											<td class="text-right"><strong><?php if($currency->position==1){echo $currency->curr_icon;}?> <?php echo $gtoal = $calvat+$itemtotal+$servicecharge-$discount-$billinfo->return_amount;?> <?php if($currency->position==2){echo $currency->curr_icon;}?> </strong></td>
										<?php endif;?>
									</tr>
                                    <?php 
			if($orderinfo->customerpaid>0){
				...
				$changes=$customepaid-$orderinfo->totalamount;
					...
					if($subOrderPayment > 0){
						...
						$changes= $subOrderPayment - $gtoal;
					}else{
						$customepaid=$orderinfo->totalamount;
						$changes=0;
					}
				}
				// $customepaid=$customepaid-
			?>
            <tr>
              <td align="right" colspan="4">
				<?php if($orderinfo->is_duepayment == 1){ ?>
					<nobr><?php echo display('customer_due_amount')?></nobr>
				<?php }else{  ?>
