...
<div class="panel-body">
	...
	<table border="0" class="font-18 wpr_100" style="width: 380px;font-size:20px;margin: 0 auto;">
          ...
          <table width="100%">
            ...
            <?php     $i=0; 
					...
					foreach ($iteminfo as $item){
						...
						$isexitsitem=$this->App_android_model->readupdate('tbl_updateitems.*,SUM(tbl_updateitems.qty) as totalqty', 'tbl_updateitems', array('ordid' =>$item->order_id,'menuid'=>$item->menu_id,'varientid'=>$item->varientid,'addonsuid'=>$item->addonsuid));
			...
			foreach ($exitsitem as $exititem){	
				...
				?>
			<tr>
			  <td align="left"><?php echo quantityshow($exititem->qty,$exititem->is_customqty);?></td>
      ...
      <?php }?>
      <tr>
      	<td align="center" style="font-size:24px;"><?php if(!empty($tableinfo)){ echo display('table').': '.$tableinfo->tablename;}?> | <?php echo display('ord_number');?>:<?php echo $orderinfo->order_id;?></td>
      </tr>
      <tr>
      <?php if($isupdateorder==1){?>
      	<td align="center"><?php echo display('date');?>:<?php echo $orderinfo->order_date.' '.date('h:i A');?></td>
        <?php }else{ ?>
