                   ...
                   <div class="panel">
                     ...
                     <div class="owl-carousel owl-theme">
                      ...
                      <div> <table class="table table-bordered table-hover bg-white wpr_100">
                                    ...
                                    <?php 
									$tableinfo=$this->ordermodel->old_counter_ongoingorderlimit($limit,$start);
									if(!empty($counterorder)){
										...
										foreach($tableinfo as $myorder){
											...
											if($myorder->cutomertype==2){
												...
												$secs = strtotime($myorder->cookedtime)-strtotime("00:00:00");
												$result = date("H:i:s",strtotime($myorder->order_time)+$secs);
                                        ...
                                        $mins = sprintf('%02d:%02d:%02d', ($diff / 3600), ($diff / 60 % 60), $diff % 60);       $st=1;
                                            $st=0;?>
                                            <script>
											 var timer<?php echo $myorder->order_id;echo $i;?> = "<?php echo $mins;?>";
											 var timer = timer<?php echo $myorder->order_id;echo $i;?>.split(':');
											 var hours<?php echo $myorder->order_id;echo $i;?> = parseInt(timer[0], 10);
											 var minutes<?php echo $myorder->order_id;echo $i;?> = parseInt(timer[1], 10);
											 var seconds<?php echo $myorder->order_id;echo $i;?> = parseInt(timer[2], 10);
											 
											 var totalSeconds<?php echo $myorder->order_id;echo $i;?> = hours<?php echo $myorder->order_id;echo $i;?>*3600+minutes<?php echo $myorder->order_id;echo $i;?>*60;
											var timerVar = setInterval(
											function() {
												 //console.log(totalSeconds<?php echo $myorder->order_id;echo $i;?>);
												++totalSeconds<?php echo $myorder->order_id;echo $i;?>;
												 var hour = Math.floor(totalSeconds<?php echo $myorder->order_id;echo $i;?> /3600);
												 var minute = Math.floor((totalSeconds<?php echo $myorder->order_id;echo $i;?> - hour*3600)/60);
												 var secondsf = totalSeconds<?php echo $myorder->order_id;echo $i;?> - (hour*3600 + minute*60);
												   if(hour < 10)
													 hour = "0"+hour;
												   if(minute < 10)
													 minute = "0"+minute;
												   if(secondsf < 10)
													 secondsf = "0"+secondsf;
													  $('.countdown_<?php echo $i;?>').html('Time Over '+hour+':'+minute + ':' + secondsf);
												}
																					, 1000);
                                       ...
                                       <tr>
                                       	  <td><strong><?php echo $myorder->tablename;?></strong></td>
                                          <td><strong><?php echo getPrefixSetting()->sales. '-' .$myorder->order_id.' '.$online;?></strong></td>
                                          <td><strong><?php echo  date('h:i:s A', strtotime($myorder->order_time));?></strong></td>
                                          <td class="font-weight"><?php if($st==1){?><div class="countdown_<?php echo $i;?>"></div><?php }else{ ?><div class="countdown_<?php echo $i;?>"></div><?php }?></td>
