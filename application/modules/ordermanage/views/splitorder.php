<link rel="stylesheet" type="text/css" href="<?php echo base_url('application/modules/ordermanage/assets/css/splitorder.css'); ?>">
    ...
    <?php ?>
            ...
            <div id="payprint_marge" class="modal-dialog modal-inner" role="document" style="color:#333;">
...
foreach ($iteminfo as $item) {
    ...
    ?>
                                                    <tr onclick='addintosuborder("<?php echo $item->row_id; ?>","<?php echo $item->order_id; ?>",this)' data-qty='<?php echo $qty; ?>' data-url="<?php echo base_url() . $module . '/order/showsuborderdetails/'; ?>">
...
echo form_dropdown('number', $list, $count, ' class="form-control" id="number-of-sub-order" onchange="showsuborder(this)" data-url="' . base_url() . $module . '/order/showsuborder/" data-value="' . $order_info->order_id . '"')?>
    ...
    foreach ($suborder_info as $suborder) {
        ...
        ?>
  <div class="col-md-6">
                                    <div class="info_part split-item" onclick="selectelement(this,<?php echo $suborder->status; ?>)" data-value="<?php echo $suborder->sub_id; ?>">
        ...
        if (!empty($suborder->order_menu_id)) {
            ...
            foreach ($suborder->suborderitem as $subitem) {
                ...
                if (!empty($subitem->add_on_id) && !empty($isaddones)) {
                    ...
                    //$addonsname = is_array($addonsname) ?? $addonsname : [];
                    $addonsnamestring = implode($addonsname, ',');

                }
                if (!empty($subitem->tpid) && !empty($isaddones)) {
                    $t            = 0;
                    $topping      = explode(',', $subitem->tpid);
                    $toppingprice = explode(',', $subitem->tpprice);
                    $tp           = 0;
                ...
                ?>
                                                ...
                                                <tr>
                                                    <td scope="row"><?php echo $subitem->ProductName . ',' . $addonsnamestring . ',' . $toppingnamestring; ?></td>
                    ...
                    echo $suborderqty[$subitem->row_id] * $subitem->price - ($suborderqty[$subitem->row_id] * $discountt) + $adonsprice + $alltpprice;
                    $totalprice = $totalprice + $suborderqty[$subitem->row_id] * $subitem->price - ($suborderqty[$subitem->row_id] * $discountt) + $adonsprice + $alltpprice;
                    $itemprice  = $suborderqty[$subitem->row_id] * $subitem->price - ($suborderqty[$subitem->row_id] * $discountt) + $adonsprice + $alltpprice;
                } else {
                    echo $suborderqty[$subitem->row_id] * $subitem->price + $adonsprice + $alltpprice;
                    $itemprice  = $suborderqty[$subitem->row_id] * $subitem->price + $adonsprice + $alltpprice;
                    $totalprice = $totalprice + $suborderqty[$subitem->row_id] * $subitem->price + $adonsprice + $alltpprice;
...
$msd = $itemprice * $settinginfo->servicecharge / 100;
        ...
        ?>
                                           ...
                                           <?php 
                                            ...
                                            $partial_vat = $this->db->select('*')->from('tax_collection')->where('relation_id', $suborder->order_id)->get()->row()->{'tax' . $i};
                                            $billinfo = $this->db->select('sub_order.*, bill.service_charge, bill.order_id, bill.bill_id, bill.VAT, bill.bill_amount')
                                                            ->from('sub_order')
                                                            ->join('bill', 'bill.order_id = sub_order.order_id')
                                                            ->where('sub_order.sub_id', $suborder->sub_id)
                                                            ->get()
                                                            ->row();
                                            $count_suborders = $this->db->from('sub_order')
                                                                    ->where('order_id', $suborder->order_id)
                                                                    ->count_all_results();
                                            $sc = $billinfo->service_charge / $count_suborders;
                                           ?>

                                           <?php if($isvatinclusive):?>
                                                <!-- inclusive table footer -->
                                                <tfoot>
                                                    <tr>
                                                        <td colspan="2" align="right" class="text-right font-14">&nbsp; <b><?php echo display('total') ?> </b></td>
                                                        <td class="text-right"><b><?php echo number_format($totalprice, 3); ?> </b></td>
                                                    </tr>

                                                    <?php if (empty($taxinfos)) {?>
                                                    <tr>
                                                        <td colspan="2" align="right" class="text-right font-14">&nbsp; <b><?php echo display('vat_tax') ?> </b></td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td colspan="2" align="right" class="text-right font-14">&nbsp; <b><?php echo $mvat['tax_name'] . '@' . $multiplletax['tax' . $i] . '%'; ?></b></td>
                                                        <td class="text-right"><b><?php echo $fp_vat = number_format(($partial_vat / ($billinfo->bill_amount - $billinfo->service_charge)) * $totalprice, $settinginfo->showdecimal);?></b></td>
                                                    ...
                                                    ?>
                                                    <tr>
                                                        <td colspan="2" align="right" class="text-right font-14">&nbsp; <b><?php echo display('service_chrg') ?> </b></td>
                                                        <td class="text-right"><b><?php echo number_format($sc, 3); ?></b></td>
                                                    </tr>


                                                    <tr>
                                                        <td colspan="2" align="right" class="text-right font-14">&nbsp; <b><?php echo display('grand_total') ?> </b></td>
                                                        <td class="text-right"><b><?php echo number_format($totalprice + $fp_vat + $sc, 3); ?></b></td>
                                                        <input type="hidden" id="total-sub-<?php echo $suborder->sub_id; ?>" value="<?php echo $totalprice; ?>">
                                                        <input type="hidden" id="vat-<?php echo $suborder->sub_id; ?>" value="<?php echo $fp_vat; ?>">
                                                        <input type="hidden" id="service-<?php echo $suborder->sub_id; ?>" value="<?php echo $sc; ?>">
                                                        <input type="hidden" id="orderid-<?php echo $suborder->sub_id; ?>" value="<?php echo $suborder->order_id; ?>">
                                                    </tr>

                                                </tfoot>
                                            <?php else:?>

                                                <!-- exclusive table footer -->
                                                <tfoot>
                                                    <tr>
                                                    ...
                                                    <tr>
                                                        ...
                                                        echo number_format($totalprice+$calvat+$sc,3);?></b></td>
                                                        <input type="hidden" id="total-sub-<?php echo $suborder->sub_id;?>" value="<?php echo $totalprice;?>">
