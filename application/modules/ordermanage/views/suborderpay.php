												...
												<ul class="list-unstyled mt-3 mb-0 w-100">
														<li>
															<div class="table-responsive px-15">
																<table class="table table-total">
																	<tbody>
																		...
																		<input name="submain" type="hidden" value="<?php echo number_format($order_sub->total_price,3);?>" id="main_subtotal" />
                                                                        </tr>
																		<tr>
																		...
																		<?php if($isvatinclusive):?>
																			...
																			<tr>
																				...
																				$partial_vat = $this->db->select('*')->from('tax_collection')->where('relation_id', $order_sub->order_id)->get()->row()->{'tax' . $i};
																				...
																				->where('sub_order.sub_id', $order_sub->sub_id)
																				->get()
																				->row();

																				$count_suborders = $this->db->from('sub_order')
																				->where('order_id', $order_sub->order_id)
																				->count_all_results();

																				$sc = $billinfo->service_charge/$count_suborders;

																				echo $fp_vat = number_format(($partial_vat/($billinfo->bill_amount-$billinfo->service_charge))*($billinfo->total_price), $settinginfo->showdecimal);
															...
															<div class="w-100 discount_area px-15">
																...
																<input type="number" class="w-40 fs-15 fw-600 amount_box" placeholder="Amount" id="discount" oninput="this.value; /^[0-9]+(.[0-9]{1,3})?$/.test(this.value)||(value='0.00');" name="discount" value="<?php echo $defaultdis; //$settinginfo->discountrate;?>" />
													...
													foreach($allpaymentmethod as $psingle){
														...
														<div id="pay_<?php echo $psingle->payment_method_id; ?>" class=" fade tab-pane <?php if($p==1){ echo "active in";}?>">
															...
															<div class="payment-content">
																...
																<div class="fw-700 d-flex align-items-center"><?php echo display('amount');?>: <input class="input-amount w-40 fs-15 fw-600 clearamount" id="getp_<?php echo $psingle->payment_method_id?>" data-pname="<?php echo $psingle->payment_method_id?>" onkeyup="changedueamount(<?php echo $psingle->payment_method_id?>)" onclick="givefocus(this)"  placeholder="Enter Amount" autocomplete="off">
																<?php if($settinginfo->currencyconverter==1){?>
	 ...
	 var csrf = $('#csrfhashresarvation').val();
	  ...
	  var url = basicinfo.baseurl + 'ordermanage/order/democonversion';
	   $.ajax({
		   type: "POST",
		   url: url,
		   data: { csrf_test_name:csrf,price:price,to_currency:to_currency},
