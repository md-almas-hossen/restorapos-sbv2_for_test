...
class Reservation extends MX_Controller
{
	...
	public function index($id = null)
	{
		...
		/*$config = array(
        			...
        			'smtp_port' => "587",
        			'smtp_user' => "edotcovms@gmail.com",
        			'smtp_pass' => "qbbhgsvqgcpugmzm",
        			'mailtype'  => "html",
        			'charset'   => 'utf-8'
        		);
                
                $subject="Test";
        			    $mailContent = "fgdggfhf";
                		$this->load->library('email',$config);
						$this->email->initialize($config);
                		$this->email->set_newline("\r\n");
                		$this->email->set_mailtype("html");
                		$this->email->from("edotcovms@gmail.com", 'Test');
                		$this->email->to("ainal1haque@gmail.com");
	...
	public function reservationform()
	{
		...
		$data['customerlist']   = $this->reservation_model->customer_dropdown();
		$data['tablelist']     = $this->reservation_model->table_dropdown();
		$data['tableinfo'] = $this->reservation_model->findBytableId($this->input->post('id'));
	...
	public function create($id = null)
	{
		...
		$redirect_status=$this->input->post('redirect_status', true);
		if ($this->form_validation->run()) {
			if (empty($this->input->post('reserveid'))) {
				...
				$data['units']   = (object) $postData = array(
					...
					'reserveday' 	 	 => $newdate,
					'foods_prereference' => $this->input->post('foods_prereference', true),
				...
				if ($this->reservation_model->create($postData)) {
                    ...
                    if($redirect_status ==1){
						redirect('reservation/reservation/reservation_dashboard');
					}else{
				...
				$data['units']   = (object) $postData = array(
					...
					'tableid' 	 		 => $data['reserveinfo']->tableid,
					'person_capicity' 	 => $this->input->post('tablicapacity', true),
					'formtime' 	 		 => $this->input->post('bookfromtime', true),
					'totime' 	 		 => $this->input->post('bookendtime', true),
					'reserveday' 	 	 => $newdate,
...
WHERE offdaydate = '$cdate'";
	...
	public function checkisbook()
	{
		...
		//echo $isbooking=$this->db->select("*")->from('rest_table')->where('tableid', $tablid)->where('status', 1)->get()->num_rows();
	...
	{
		// $this->permission->method('reservation','read')->redirect();
		$data['title']    = display('reservation_dashboard');
		// $data["setting"] = $this->reservation_model->read('*', 'setting', array('id' => 2));
		$data['module'] = "reservation";
		$data['page']   = "reservation_calender";
		echo Modules::run('template/layout', $data);
	}

	public function showreservationorderjs()
	{
		$this->db->select('*');
		...
		foreach ($orderinfo as $order) {
			...
			$reserveday = date("Y-m-d", strtotime($order->reserveday));
			$data[] = array(
				'title'       => 'Reserve id :' . $order->reserveid . ', Start Time : ' . $order->formtime . ',End Time:' . $order->totime . ' ' . ',Person:' . $order->person_capicity,
				'start'       => $reserveday,
				'mydate'      => $reserveday,
				'constraint'  => $order->person_capicity,
			);
		}
	...
	public function reservation_show_order()
	{

		$reserveday = $this->input->post('reserveday');
		$condition = "DATE(tblreservation.reserveday)='" . $reserveday . "'";
		...
		echo $this->load->view('reservation_show_order', $data);
	}
	public function avalieablefrom()
	{

		$this->permission->method('reservation', 'read')->redirect();
		$data['title'] = display('take_reservation');
		echo $this->load->view('reservation_avalieablefrom');
	}
}
