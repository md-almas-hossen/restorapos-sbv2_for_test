...
$('body').on('click', '#changedelivarystatus', function() {
				     var url = basicinfo.baseurl+'ordermanage/order/save_pickup';					 
					 var csrf = $('#csrfhashresarvation').val();
					 var order_id=$("#order_id").val();
					 var thirdparty_id=$("#thirdparty_id").val();
					 ...
					 if (ridername == '') {
								toastr.error('Rider Name is Required', 'Error');
								$("#ridername").focus();
								return false;
					}
					if(status==3){
						...
						if(paymentmethod==1 && bankid==''){
							toastr.error('Please Select Bank!!!', 'Error');
								$("#bankid").focus();
								return false;
						}
						if(paymentmethod==14 && mobilelist==''){
							toastr.error('Please Select Mobile Method Name!!!', 'Error');
								$("#bankid").focus();
								return false;
						}
					}
					 ...
					 $.ajax({
							 type: "POST",
							 url: url,
							 data:{csrf_test_name:csrf,order_id:order_id,thirdparty_id:thirdparty_id,delivery_time:delivery_time,status:status,ridername:ridername,paymentmethod:paymentmethod,bankid:bankid,terminal:terminal,last4digit:last4digit,mobilelist:mobilelist,mobile:mobile,transactionno:transactionno},
							 success: function(data) {	
									...
									$(".customer_delivery").hide();
									}else if(status == 3){
										$('.account_receive').hide();
									}	
  								  // website order pickup ,delivery
								 ...
								 if(data==1){								
									...
									if(pagename==1){
									 $("#thirdparty_order").trigger('click');
									}else{
