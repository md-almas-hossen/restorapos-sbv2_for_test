...
class Catering_model extends CI_Model
{
	...
	public function categoryListPackageWise($package_id)
	{
		$data = $this->db->select('*')->from('item_category ic')
			->join('tbl_package_details pd', 'ic.CategoryID=pd.category_id', 'left')
	...
	public function category_wise_foods($category_id)
	{
		...
		foreach ($query as $value) {
			$option .= "<option value='" . $value->ProductsID . "'data-price='" . $value->price . "'data-productvat='" . $value->productvat . "'data-variantid='" . $value->variantid . "'>" . $value->ProductName . '(' . $value->variantName . ')' . "</option>";
		}
		return $option;
	}


	public function foodPrice($food_id, $variant_id)
	{
		$query = $this->db->select('item_foods.*,variant.variantid,variant.variantName,variant.price')
			->from('item_foods')
			->join('variant', 'item_foods.ProductsID=variant.menuid', 'left')
			->where('ProductsIsActive', 1)
			->where('item_foods.ProductsID', $food_id)
			->where('variant.variantid', $variant_id)
			->where('item_foods.is_packagestatus', 0)
			->where('item_foods.isgroup', null)
			->get()
			->row();
	...
	public function catering_item($orderid)
	{
		...
		$package_id = $this->input->post('package_id'); // have to modify
		$pack_qty = $this->input->post('package_qty');
		...
		$price = $this->input->post('price'); // have to modify

		$variantid = $this->input->post('variantid'); // have to get these datas
		$product_vat = $this->input->post('product_vat'); // have to get these datas
		...
		for ($i = 0; $i < count($ProductsID); $i++) {
			...
			$iteminfo = $this->getiteminfoUpdated($ProductsID[$i], $variantid[$i]);
			...
			if ($iteminfo->isgroup == 1) {
				...
				foreach ($groupinfo as $grouprow) {
					...
					$data3 = array(
						...
						'itemdiscount'			=> (($package_id[$i] == 'none_package') ? $iteminfo->OffersRate : 0.00),
						'addonsuid'	        	=>	NULL,
						'add_on_id'	        	=>	'',
						'addonsqty'	        	=>	'',
						'tpassignid'	        =>	'',
				...
				if ($iteminfo->isopenfood == 1) {
					...
					$data3 = array(
						...
						'is_package'            =>  $is_package,
						'package_id'            =>  $pkid,
						'notes'					=>  $iteminfo->itemnote,
						'menuqty'	        	=>	$qty[$i],
						// 'price'	        		=>	$price[$i],
						// 'price'	        		=>	$itemprice,
						'price'	        		=> (($package_id[$i] == 'none_package') ? $singleItemprice : $prices),
						'itemdiscount'			=> (($package_id[$i] == 'none_package') ? $iteminfo->OffersRatee : 0.00),
						'addonsuid'	        	=>	NULL,
						'add_on_id'	        	=>	'',
						'addonsqty'	        	=>	'',
						'tpassignid'	        =>	'',
		...
		$payment = empty($this->input->post('card_type'))?4:$this->input->post('card_type');
		if (!empty($payment)) {
			$discount = $pdiscount;
			if ($vat == '') {
				$vat = 0;
	...
	public function catering_order_update($orderid)
	{
		...
		for ($i = 0; $i < count($package_id); $i++) {
			...
			// $itemsubtotal = $itemsubtotal + $nittotal + $price[$i] * (($package_id[$i] == 'none_package') ? $qty[$i] : 1);
			$itemsubtotal = $itemsubtotal + $nittotal + $price[$i];
	...
	// check this method used anywhere or not, if not then it has be to be deleted.
	...
	public function getiteminfoUpdated($id = null, $variant_id)
	{
		$this->db->select('*');
		$this->db->from('item_foods');
		$this->db->join('variant', 'item_foods.ProductsID=variant.menuid', 'left');
		$this->db->where('ProductsID', $id);
		$this->db->where('variant.variantid', $variant_id);
		$this->db->where('ProductsIsActive', 1);
		// $this->db->order_by('itemordering','ASC');
		$query = $this->db->get();
		$itemlist = $query->row();
