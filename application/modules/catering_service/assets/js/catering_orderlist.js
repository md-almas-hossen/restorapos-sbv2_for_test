...
$(document).ready(function () {
  ...
  var orderlist = $("#tallorder").DataTable({
    ...
    ajax: {
      url:
        basicinfo.baseurl +
        "catering_service/cateringservice/allcatering_orderlist",
      type: "post",
      data: function (data) {
...
function createMargeorder(orderid, value = null) {
  var url =
    basicinfo.baseurl +
    "catering_service/cateringservice/showpaymentmodal/" +
    orderid;
  callback = function (a) {
    $("#modal-ajaxview").html(a);
    $("#payprint_marge").on("hide.bs.modal", () => {
      $("#modal-ajaxview").html("");
    });
...
function submitmultiplepay() {
  ...
  if (noinputvalue == 0) {
    ...
    var test =
      '<input class="emptysl" name="paytype[]" data-total="' +
      paidvalue +
      '" type="hidden" value="' +
      selecttab +
      '" id="ptype_' +
      selecttab +
      '" /><input type="hidden" id="paytotal_' +
      selecttab +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (value == 1) {
        ...
        var page_url2 =
          basicinfo.baseurl +
          "catering_service/cateringservice/catering_orderlist";
...
function printPosinvoice(id) {
  ...
  var url =
    basicinfo.baseurl +
    "catering_service/cateringservice/posorderinvoice/" +
    id;
  $.ajax({
    type: "GET",
    url: url,
