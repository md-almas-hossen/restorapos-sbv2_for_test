...
class Printer_model extends CI_Model {
	...
	public function allcounterprinters(){
			$this->db->select('*');
			$this->db->from('tbl_printersetting');
			$this->db->where("ipaddress!=''");
			$this->db->order_by('counterno', 'desc');
			$query = $this->db->get();
			if ($query->num_rows() > 0) {
				return $query->result();    
			}
	...
	public function findByIdcp($id = null)
	{ 
		return $this->db->select("*")->from('tbl_printersetting')
			->where('pid',$id) 
			->get()
			->row();
	} 
	
	public function createprinter($data = array())
	{
		return $this->db->insert('tbl_printersetting', $data);
	}
	public function updateprinter($data = array())
	{
		return $this->db->where('pid',$data["pid"])
			->update('tbl_printersetting', $data);
	}
	public function deleteprinter($id = null)
	{
		$this->db->where('pid',$id)
			->delete('tbl_printersetting');
		if ($this->db->affected_rows()) {
			return true;
		} else {
			return false;
