...
class Item_category extends MX_Controller {
    ...
    public function create($id = null,$page=null)
    {
	  ...
	  $img = $this->fileupload->do_upload(
	   ...
	   $caticon = $this->fileupload->do_upload(
		'./application/modules/itemmanage/assets/images/','caticon'
	  ...
	  #-------------------------------#
		...
		$lastid=$this->db->select("*")->from('item_category')
		->order_by('CatCode','desc')
		->get()
		->row();
		$sls=$lastid->CatCode;
		if(empty($sls)){
	  ...
	  if ($this->form_validation->run()) { 
	   ...
	   'showonweb'           =>$notvisible,
	   'ordered_pos'        =>$this->input->post('ordering',true),
	   'CategoryIsActive'   => $this->input->post('status',true),
	   'catcolor'   		=> $this->input->post('catcolor',true),
	   'caticon'   			=> $caticon,
	   'isoffer'     		=> $isoffer, 
	...
	// dd($sino);
	 ...
	 $logData = array(
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		if(empty($caticon)){
			$caticon=$this->input->post('old_icon');
			}
		$data['category']   = (Object) $postData = array(
	   'CategoryID'     => $this->input->post('CategoryID',true),
	   'Name'     	=> $this->input->post('categoryname',true), 
