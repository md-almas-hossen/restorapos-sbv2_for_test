...
class Menu_addons extends MX_Controller {
    ...
    public function create($id = null)
    {
	  ...
	  $existingredients=$this->db->select('ingredient_name')->from('ingredients')->where('ingredient_name',$this->input->post('addonsname',true))->where($condition)->get()->row();
	   ...
	   $lastid = $this->db->select("*")->from('add_ons')
						->order_by('addonCode','desc')
						->get()
						->row();

						$sls = $lastid->addonCode;
						if(empty($sls)){
							$sl = "Addon-0000";
						}else{
							$sl = $sls;
	   ...
	   'entry_date'          => date('Y-m-d H:i:s'),
		...
		$this->logs_model->log_recorded($logData);
			if(empty($existingredients)){
				$updatetype = array(
						"ingredient_name"           => $this->input->post('addonsname',true),
				...
				$this->db->where('ingredient_name',$this->input->post('addonsname',true));
	  ...
	  $data['addons']   = (Object) $postData = [
	   ...
	   // This block of code was not executed when  updating addons, where TAX info will be updated
	   $taxsettings = $this->taxchecking();
		if(!empty($taxsettings)){
			$tx=0;
			$taxitems= array();
		...
		$this->logs_model->log_recorded($logData);
			...
			if(empty($existingredients)){
				$updatetype = array(
						...
						"is_addons"           		=> 1,
						"is_active"           		=> $this->input->post('status',true),
						"itemid"					=> $this->input->post('add_on_id',true)
				        );
				$this->db->insert('ingredients', $updatetype);
			}
			else{
    ...
    public function assignaddonscreate($id = null)
    {
	  ...
	  $lastid = $this->db->select("*")->from('menu_add_on')
						->order_by('assignCode','desc')
						->get()
						->row();
	  ...
	  if ($this->form_validation->run()) { 
	   ...
	   $data['addons']   = (Object) $postUpdateData = [
		   'row_id'     => $this->input->post('row_id',true),
		   'add_on_id'     => $this->input->post('addonsid',true),
		   'menu_id'     	=> $this->input->post('menuid',true), 
		   'is_active'   => 1,
		...
		if ($this->addons_model->update_menuaddons($postUpdateData)) { 
