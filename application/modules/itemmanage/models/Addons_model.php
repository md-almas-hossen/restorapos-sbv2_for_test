...
class Addons_model extends CI_Model {
	...
	{
		$this->db->where('add_on_id',$id)->where('istopping',0)->delete($this->table);
  ...
  public function menuaddons_delete($id = null)
	...
	{
		return $this->db->where('add_on_id',$data["add_on_id"])->where('istopping',0)->update($this->table, $data);
	}
	public function update_menuaddons($data = array())
	{
		
		return $this->db->where('row_id',$data["row_id"])->update('menu_add_on', $data);
	}
	...
	public function read_menuaddons($limit = null, $start = null)
	{
		...
		$this->db->where('add_ons.istopping',0);
