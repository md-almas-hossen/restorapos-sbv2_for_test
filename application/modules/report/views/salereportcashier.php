<script src="<?php echo base_url('application/modules/report/assets/js/salereportcashier.js'); ?>" type="text/javascript"></script>
<link href="<?php echo base_url('application/modules/report/assets/css/salereportfrm.css'); ?>" rel="stylesheet" type="text/css"/>

<div class="row">
        <div class="col-sm-12 col-md-12">
            <div class="panel">
               
                <div class="panel-body">
                    <fieldset class="border p-2">
                       <legend  class="w-auto"><?php echo $title; ?></legend>
                    </fieldset>
                    <div class="row">
			<div class="col-sm-12">
		        <div class="panel panel-default">
		            <div class="panel-body"> 
		                <?php echo form_open('report/index',array('class' => 'form-inline'))?>
		                <?php $today = date('Y-m-d'); ?>
		                    <div class="form-group">
		                        <label class="" for="from_date"><?php echo display('start_date') ?></label>
		                        <input type="text" name="from_date" class="form-control datepicker5" id="from_date" value="<?php echo date('d-m-Y');?>" placeholder="<?php echo display('start_date') ?>" readonly="readonly" >
		                    </div> 

		                    <div class="form-group">
		                        <label class="" for="to_date"><?php echo display('end_date') ?></label>
		                        <input type="text" name="to_date" class="form-control datepicker5" id="to_date" placeholder="<?php echo "To"; ?>" value="<?php echo $today?>" readonly="readonly">
		                    </div> 
                            <?php $loguid=$this->session->userdata('id');
								$isAdmin=$this->session->userdata('user_type');
								if($isAdmin==1){
					        ?>
                            <div class="form-group">
                                 <select name="salesman" class="form-control" id="salesman">
                                    <option value="" selected="selected">Select Salesman</option>
                                    <?php foreach($salesmanlist as $row){?>
                                    <option value="<?php echo $row->create_by;?>"><?php echo $row->CashierName;?></option>
                                    <?php } ?>
                                    
                                </select>
		                    </div>
                            <?php }else{ ?>
                             <input type="hidden" name="salesman" id="salesman" value="<?php echo $loguid?>" >
                            <?php } ?>
		                    <a  class="btn btn-success" onclick="getreport()"><?php echo display('search') ?></a>
                          
		                    <a  class="btn btn-warning" href="#" onclick="printDiv('purchase_div')"><?php echo display('print') ?></a>
		                   </div>
		                  
 							
		               <?php echo form_close()?>
		            </div>
		        </div>
		    </div>
	    </div>
					<div class="row">
		    <div class="col-sm-12">
		        <div class="panel panel-bd">
		            <div class="panel-heading">
		                <div class="panel-title">
		                    <h4><?php echo display('sell_report') ?></h4>
		                </div>
		            </div>
		            <div class="panel-body">
		            	<div id="purchase_div">
			            	<div class="text-center" id="htop">
								<h3> <?php echo $setting->storename;?> </h3>
								<h4><?php echo $setting->address;?> </h4>
								<h4><?php echo $setting->phone;?> </h4>
								<h4> <?php echo "Print Date" ?>: <?php echo date("d/m/Y h:i:s"); ?> </h4>
								<h4 id="hsdate" style="display:none;">Date: <span id="sdate"></span> </h4>
								<h4><?php echo $title; ?></h4>
							</div>
			                <div class="table-responsive" id="getresult2">
			                    
			                </div>
							<div class="text-center">
                                  <h4><?php echo $setting->powerbytxt;?></h4>
                            </div>
			            </div>
			            <div class="text-right">
			            </div>
		            </div>
		        </div>
		    </div>
		</div>
                </div> 
            </div>
        </div>
    </div>

