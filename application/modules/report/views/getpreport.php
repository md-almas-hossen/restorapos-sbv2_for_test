...
<table class="table table-bordered table-striped table-hover" id="respritbl">
	<thead>
		<tr class="voucherList">
			...
			<th style="background: #abbff9!important"><?php echo display('supplier_name') ?></th>
			<th style="background: #abbff9!important"><?php echo display('total_ammount') ?></th>
		</tr>
	</thead>
	<tbody class="getpreport">
		...
		?>
				<tr class="<?php echo $k % 2 ? 'voucherList' : '' ?>">
					...
					<td style="background:<?php echo $style; ?>"><?php echo $pitem->supName; ?></td>
					<td style="background:<?php echo $style; ?>" class="total_ammount">
						<?php if ($currency->position == 1) {
							echo $currency->curr_icon;
						} ?>
						<?php
						echo numbershow($pitem->total_price, $setting->showdecimal);
						?>
	...
	<tfoot class="prechasereport-footer">
		...
		<!-- <tr> 
		    <td>&nbsp;</td>
			<td>&nbsp;</td>
			<td class="total_purchase" align="right">&nbsp; <b><?php echo display('paid') ?> </b></td>
		...
		<!-- <tr> -->
			...
			<!-- 
			<td class="total_purchase" align="right">&nbsp; <b><?php //echo display('invoice') . ' ' . display('total_due') ?> </b></td>
			<td class="totalprice" align="right">
				<b>
					<?php //if ($currency->position == 1) {
						//echo $currency->curr_icon;
