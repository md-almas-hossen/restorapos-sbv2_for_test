...
<table class="table table-bordered table-striped table-hover" id="respritbl">
									...
									$getOrderPaymentCheck =$this->db->select('IFNULL(SUM(pay_amount), 0) as totalpaid')->from('order_payment_tbl')->where('order_id',$pitem->order_id)->get()->row();
									...
									echo $newDate = date("d-M-Y", strtotime($originalDate));
									 ?></td>	<td><?php echo $newDate = date("h:i A", strtotime($pitem->order_time));?></td>
												...
												<td class="total_ammount">
													<?php 
													if($currency->position==1){echo  $currency->curr_icon.' ';}
													 echo numbershow($getOrderPaymentCheck->totalpaid, $setting->showdecimal);
													if($currency->position == 2){echo ' '. $currency->curr_icon;}
													?>
												</td>
												<td class="total_ammount">
													...
													<?php 
														echo numbershow(($pitem->bill_amount-($pitem->return_amount+$getOrderPaymentCheck->totalpaid)), $setting->showdecimal);
													?>
