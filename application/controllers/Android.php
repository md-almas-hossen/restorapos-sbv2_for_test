...
class Android extends MY_Controller {
	...
	public function sign_up()
    {
		 ...
		 if(empty($this->input->post('email', TRUE)) && (!empty($this->input->post('mobile', TRUE)))){
		  ...
		  $setinginfo=$this->db->select("issmsenable")->from('setting')->where('id',2)->get()->row();
				if($setinginfo->issmsenable==1){
				    $isactiveac=0;
				}else{
				   $isactiveac=1; 
				}
            ...
            else
            {
				...
				$data['is_active']    				= $isactiveac;
                $data['customer_picture']    		= $image;
                $data['favorite_delivery_address']  = $this->input->post('favouriteaddress', TRUE);
                $data['otpcode']  = $otp;
				 if(!empty($exitsinfo)){
					 $updatetData['otpcode']    = $otp;
				   	 $update = $this->Api_v2_model->update_date('customer_info', $updatetData, 'customer_id', $exitsinfo->customer_id);
                ...
                if ($insert_ID) {
                    ...
                    $output = $this->Api_v2_model->read("customer_id,cuntomer_no,membership_type,customer_name,customer_email,password,customer_token,customer_address,customer_phone,customer_picture,favorite_delivery_address,crdate,is_active", 'customer_info', array('customer_id' => $insert_ID));
                    ...
                    $output->{"smsisactive"} = $setinginfo->issmsenable;
                     $c_name = $this->input->post('customer_name');
					   ...
					   $createdate=date('Y-m-d H:i:s');
						...
						$this->Api_v2_model->insert_data('acc_subcode', $postData1);
	     ...
	     public function checkotp()
    {
            ...
            else
            {
				...
				$custinfo = $this->Api_v2_model->read('*', 'customer_info', array('customer_id' => $Customerid, 'otpcode' => $otp));
				if(!empty($custinfo)){
				    $updatetData['is_active']    = 1;
				   	$update = $this->Api_v2_model->update_date('customer_info', $updatetData, 'customer_id', $custinfo ->customer_id);
				return $this->respondWithSuccess('OTP Verification Successfully:', $output);
				}
				else{
					return $this->respondWithError('OTP Mismatch!!.OTP  Verificatio Failed!!!',$output);
					}
			}
	...
	public function resendotp()
    {
            ...
            else
            {
				...
				if(!empty($custinfo)){
					$this->lsoft_setting->send_sms($otp,$custinfo ->customer_id,$type="otp");
				    $updatetData['otpcode']    = $otp;
				   	$update = $this->Api_v2_model->update_date('customer_info', $updatetData, 'customer_id', $custinfo ->customer_id);
				return $this->respondWithSuccess('OTP Resend Successfully:', $output);
				}
				else{
					return $this->respondWithError('OTP Resend Failed!!!',$output);
					}
			}
            ...
            else
            {
				...
				$update = $this->Api_v2_model->update_date('customer_info', $updatetData, 'customer_id', $custinfo ->customer_id);
				 ...
				 if ($update){
				   ...
				   // print_r($output);
                    $output->{"UserPictureURL"} = base_url().$image;
					$newhead=$this->input->post('customer_name');
					$coa_update=array('name'        => $newhead);
					$this->db->where('refCode',$custinfo->customer_id);
				...
				$update = $this->Api_v2_model->update_date('customer_info', $updatetData, 'customer_id', $custinfo ->customer_id);
				 ...
				 if ($update){
				    $output = $this->Api_v2_model->read('customer_id,cuntomer_no,membership_type,customer_name,customer_email,password,customer_token,customer_address,customer_phone,customer_picture,favorite_delivery_address,crdate,is_active', 'customer_info', array('customer_id' => $custinfo ->customer_id));
				   // print_r($output);
                    $output->{"UserPictureURL"} = base_url().$image;
					$newhead=$this->input->post('customer_name');
					$coa_update=array('name'        => $newhead);
	...
	public function customerinfo()
    {
            ...
            else
            {
				$output=array();
				$Customerid=$this->input->post('Customerid');
				$custinfo = $this->Api_v2_model->read('customer_id,cuntomer_no,membership_type,customer_name,customer_email,password,customer_token,customer_address,customer_phone,customer_picture,favorite_delivery_address,crdate,is_active', 'customer_info', array('customer_id' => $Customerid));
				if(!empty($custinfo)){
	...
	public function generateNumericOTP($n) { 
	 ...
	 public function foodlist()
     {
            ...
            else
            {
				...
				$limit=10;*/
				$allfoods = $this->Api_v2_model->allfooditem($CategoryID,$start=null,$limit=null);
   ...
   public function searchproduct()
	...
	public function placeorder(){
		  ...
		  if($paymentsatus==5){
			 ...
			 /*if($socketactive->socketenable==1){
				//start send to socket
					$outputapp=array();
				$outputapp['status']='success';
				$outputapp['status_code']=1;
				$outputapp['message']='Success';
				$outputapp['type']='Token';
				$outputapp['tokenstatus']='new';
		...
		$customerinfo=$this->Api_v2_model->read('*', 'customer_info', array('customer_id' => $row->customer_id));
		if(!empty($row->waiter_id)){
		$waiter=$this->Api_v2_model->read('*', 'user', array('id' => $row->waiter_id));
		}else{
			$waiter='';
		}
		...
		$settinginfo=$this->Api_v2_model->read('*', 'setting', array('id' => 2));
		$outputapp['orderinfo'][$o]['title']=$settinginfo->title;
	    $outputapp['orderinfo'][$o]['token_no']=$row->tokenno;
		$outputapp['orderinfo'][$o]['ordertime']=date('h:i:s A');
		$outputapp['orderinfo'][$o]['orderdate']=date('d/m/Y', strtotime($row->order_date));
		$outputapp['orderinfo'][$o]['order_id']=$row->order_id;
		$outputapp['orderinfo'][$o]['customerName']=$customerinfo->customer_name;
		$outputapp['orderinfo'][$o]['customerPhone']=$customerinfo->customer_phone;
		$outputapp['orderinfo'][$o]['ordernotes']=$row->customer_note;
		if(!empty($waiter)){
		$outputapp['orderinfo'][$o]['waiter']=$waiter->firstname.' '.$waiter->lastname;
		}else{
		$outputapp['orderinfo'][$o]['waiter']='';
		}
		if(!empty($row->table_no)){
		$tableinfo=$this->Api_v2_model->read('*', 'rest_table', array('tableid' => $row->table_no));
		$outputapp['orderinfo'][$o]['tableno']=$tableinfo->tableid;
		$outputapp['orderinfo'][$o]['tableName']=$tableinfo->tablename;
		}
		else{
		$outputapp['orderinfo'][$o]['tableno']='';
		$outputapp['orderinfo'][$o]['tableName']=''; 
		}
		$k=0;
		foreach($kitchenlist as $kitchen){
		    $isupdate=$this->Api_v2_model->read('*', 'tbl_apptokenupdate', array('ordid' => $row->order_id));
		    
		
			$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['kitchenName']=$kitchen->kitchen_name;
			$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['ip']=$kitchen->ip;
			$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['port']=$kitchen->port;
			
			$i=0;
			if(!empty($isupdate)){
			    $iteminfo=$this->Api_v2_model->customerupdateorderkitchen($row->order_id,$kitchen->kitchen_id);
			    if(empty($iteminfo)){
    			$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['isitemexist']=0;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo']=array();
    			}else{
    			  $outputapp['orderinfo'][$o]['kitcheninfo'][$k]['isitemexist']=1;
				  $output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo']=array();
    			} 
				...
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['itemName']=$item->ProductName;
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['variantName']=$item->variantName;
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['qty']=$qty;
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['notes']=$item->itemnotes;
				if(!empty($item->addonsid)){
					$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['isaddons']=1;
					...
					$p=0;
						foreach($addons as $addonsid){
							...
							$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['addonsinfo'][$p]['add_onsName']=$adonsinfo->add_on_name;
							$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['addonsinfo'][$p]['add_onsqty']=$addonsqty[$p];
							$p++;
						}
					}
				else{
					$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['isaddons']=0;
					$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['addonsinfo'][0]['add_onsName']="";
					$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['addonsinfo'][0]['add_onsqty']="";
					}
				$i++;
			}
			}else{
			    $iteminfo=$this->Api_v2_model->customerorderkitchen($row->order_id,$kitchen->kitchen_id);
			    if(empty($iteminfo)){
    			$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['isitemexist']=0;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo']=array();
    			}else{
				...
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['itemName']=$item->ProductName;
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['variantName']=$item->variantName;
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['qty']=$item->menuqty;
				$outputapp['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['notes']=$item->notes;
	    ...
	    $new=array();
	    $outputapp=array('status' => 'success','type' => 'Token','tokenstatus' => 'new','status_code' => 0,'message' => 'Success','orderinfo' => $new);
	    
	}
	$senttosocket=json_encode($outputapp,JSON_UNESCAPED_UNICODE);
	send($senttosocket);
					//end	
				}*/
		    $scan = scandir('application/modules/');
