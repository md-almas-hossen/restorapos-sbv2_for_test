...
class App extends MY_Controller {
   ...
   /*Push Notification*/
								 	...
								 	foreach($payamonts  as $payamont) {
										...
										if($vatrest>$billinfo->service_charge){
											...
											if(($cptype!=1) && ($cptype!=14)){
												if($cptype==4){$headcode=$predefine->CashCode;
								...
								if($newpaytype==$carryptypeforsc){
								  ...
								  //echo $this->db->last_query();
									...
									if(($newpaytype!=1) && ($newpaytype!=14)){
											if($newpaytype==4){$headcode=$predefine->CashCode;$naration="Cash in Hand";}
													else{
													  $naration="Cash in Online";
	...
	public function printtoken(){
		...
		$waiter=$this->App_desktop_model->read('*', 'user', array('id' => $row->waiter_id));
		}else{
			$waiter='';
		}
		...
		foreach($kitchenlist as $kitchen){
		    $isupdate=$this->App_desktop_model->read('*', 'tbl_apptokenupdate', array('ordid' => $row->order_id));
			...
			$i=0;
			if(!empty($isupdate)){
			    $iteminfo=$this->App_desktop_model->customerupdateorderkitchen($row->order_id,$kitchen->kitchen_id);
			    if(empty($iteminfo)){
	...
	public function tokensendtoqueue(){
	    $this->load->library('form_validation');
		     $this->form_validation->set_rules('order_id','order_id','required');
			 $this->form_validation->set_rules('iteminfo','iteminfo','required');
			 if ($this->form_validation->run() == FALSE){
                $errors = $this->form_validation->error_array();
                return $this->respondWithValidationregisError($errors);
            }
            else
            {
				...
				$json = $this->input->post('iteminfo', TRUE);
                $itemarray = json_decode($json);
				foreach($itemarray as $items) {
					...
					$quantity=$items->qty;
					$notes=$items->notes;
					$tokenid=$items->printer_token_id;
					$tokenlog=$items->printer_status_log;
					if($items->isaddons>0){
						$adsqty='';
						$adsid='';
						foreach($items->addonsinfo as $addonsrow){
							$addons=$addonsrow->add_onsName;
							$add_onsqty=$addonsrow->add_onsqty;
							$addonsinfo=$this->db->select('add_ons.add_on_id,menu_add_on.menu_id')->from('add_ons')->join('menu_add_on','menu_add_on.add_on_id=add_ons.add_on_id')->where('add_ons.add_on_name',$addons)->get()->row();
							//echo $this->db->last_query();
							$adsqty.=$add_onsqty.',';
						    $adsid.=$addonsinfo->add_on_id.',';
							}
						$adsqty=rtrim($adsqty,',');
						$adsid=rtrim($adsid,',');
					...
					/*if(empty($foundrow)){
							...
							$qty=$quantity;
							$isdelete=NULL;
								$queuetoken = array('ordid' => $orderid,'menuid'=>$itemid,'itemnotes'=>$notes,'qty'=>$qty,'adonsqty'=>$adsqty,'addonsid'=>$adsid,'varientid'=>$variantid,'previousqty'=>$prevqty,'isdel'=>$isdelete,'isprint'=>0);
								$this->db->insert('tbl_apptokenupdate',$queuetoken);
								
					}*/
					$queuetoken = array('ordid' => $orderid,'menuid'=>$itemid,'itemnotes'=>$notes,'qty'=>$qty,'adonsqty'=>$adsqty,'addonsid'=>$adsid,'varientid'=>$variantid,'previousqty'=>$prevqty,'isdel'=>$isdelete,'isprint'=>0,'printer_token_id'=>$tokenid,'printer_status_log'=>$tokenlog);
					$updateapp = array('printer_status_log' => $tokenlog);
					$this->db->where('ordid', $orderid);
					$this->db->where('menuid', $itemid);
					$this->db->where('varientid', $variantid);
					//$this->db->where('printer_token_id', $tokenid);
					$this->db->update('tbl_apptokenupdate', $updateapp);
				} 
					return $this->respondWithSuccess('Printing Token In Queue.', $output);
            }
	}
	public function tokensendtodone(){
	         $this->load->library('form_validation');
		     $this->form_validation->set_rules('order_id','order_id','required');
			 $this->form_validation->set_rules('iteminfo','iteminfo','required');
			 if ($this->form_validation->run() == FALSE){
            ...
            else
            {
				...
				foreach($itemarray as $items) {
					...
					return $this->respondWithSuccess('Printing Token Is Done!!!.', $output);
            }
	}
	public function printtokencheck(){
	    ...
	    $output=array();
	    $kitchenlist=$this->db->select('kitchenid as kitchen_id,kitchen_name,ip,port')->from('tbl_kitchen')->order_by('kitchen_name','Asc')->get()->result();
	    $orderinfo=$this->App_desktop_model->read_allapioneday('*', 'customer_order','order_id',$date,'tokenprint','0');
		...
		foreach($kitchenlist as $kitchen){
			...
			if(!empty($isupdate)){
    			...
    			foreach($iteminfo as $item){
					$qty=0;
					$getqty=$this->App_desktop_model->read('SUM(tbl_apptokenupdate.add_qty) as cqty,SUM(tbl_apptokenupdate.del_qty) as pqty,tbl_apptokenupdate.isdel', 'tbl_apptokenupdate', array('ordid' => $item->ordid,'menuid' => $item->ProductsID,'varientid' => $item->variantid,'addonsuid' => $item->addonsuid,'isprint' => 0));
					//echo $this->db->last_query();
					
					$notes=$item->itemnotes;
					if($item->itemnotes=="deleted"){
						$notes="";
					}
					$sign="";
				...
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['variantid']=$item->variantid;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['variantName']=$item->variantName;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['qty']=$sign.$qty;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['notes']=$notes;
				if(!empty($item->addonsid)){
					$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['isaddons']=1;
					$addons=explode(",",$item->addonsid);
					$addonsqty=explode(",",$item->adonsqty);
	...
	public function printtokcompletelist(){
	    ...
	    $output=array();
	    $kitchenlist=$this->db->select('kitchenid as kitchen_id,kitchen_name,ip,port')->from('tbl_kitchen')->order_by('kitchen_name','Asc')->get()->result();
	    $orderinfo=$this->App_desktop_model->read_allapioneday('*', 'customer_order','order_id',$date,'tokenprint','1');
		$o=0;
		if(!empty($orderinfo)){
		foreach($orderinfo as $row){
		$customerinfo=$this->App_desktop_model->read('*', 'customer_info', array('customer_id' => $row->customer_id));
		...
		foreach($kitchenlist as $kitchen){
			...
			foreach($iteminfo as $item){
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['itemName']=$item->ProductName;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['itemID']=$item->menu_id;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['variantid']=$item->variantid;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['variantName']=$item->variantName;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['qty']=$item->menuqty;
				$output['orderinfo'][$o]['kitcheninfo'][$k]['iteminfo'][$i]['notes']=$item->notes;
	...
	public function printtokenorderid($id){
	    ...
	    $output=array();
	    $kitchenlist=$this->db->select('kitchenid as kitchen_id,kitchen_name,ip,port')->from('tbl_kitchen')->order_by('kitchen_name','Asc')->get()->result();
	    $orderinfo=$this->App_desktop_model->read('*', 'customer_order', array('order_id' => $id));
		if(!empty($orderinfo)){
		$customerinfo=$this->App_desktop_model->read('*', 'customer_info', array('customer_id' => $orderinfo->customer_id));
		...
		$tableinfo=$this->App_desktop_model->read('*', 'rest_table', array('tableid' => $orderinfo->table_no));
		...
		$k=0;
		foreach($kitchenlist as $kitchen){
			$iteminfo=$this->App_desktop_model->customerorderkitchen($orderinfo->order_id,$kitchen->kitchen_id);
			...
			foreach($iteminfo as $item){
				...
				$output['kitcheninfo'][$k]['iteminfo'][$i]['itemID']=$item->menu_id;
				$output['kitcheninfo'][$k]['iteminfo'][$i]['variantid']=$item->variantid;
	...
	public function printerorderupdate(){
	       $this->load->library('form_validation');
            ...
            else
            {
				 $output = array();
				 $orderid   = $this->input->post('order_id', true);
				 //$itemid   = $this->input->post('order_id', true);
				    $output['orderid'] =$orderid;
	...
	public function checkupdate(){
         ...
         */
            ...
            /*$this->output->set_content_type('text/event-stream')->set_output(json_encode($new));
	...
	public function kitchenlist(){
		$kitchenlist=$this->db->select('kitchenid as kitchen_id,kitchen_name,ip,port')->from('tbl_kitchen')->order_by('kitchen_name','Asc')->get()->result();
		 $output = array();
		 if(!empty($kitchenlist)){
			 	$k=0;
			 	foreach($kitchenlist as $kitchen){
					$output['kitcheninfo'][$k]['kitchenId']=$kitchen->kitchen_id;
					$output['kitcheninfo'][$k]['kitchenName']=$kitchen->kitchen_name;
					$output['kitcheninfo'][$k]['kitchenIp']=$kitchen->ip;
					$output['kitcheninfo'][$k]['kitchenPort']=$kitchen->port;
					$k++;
					}
				return $this->respondWithSuccess('Kitchen List.', $output);
			 }else{
				 return $this->respondWithError('Kitchen Not Found .!!!',$output);
				 }
